...
The directions can be compared, but the magnitudes must not be pooled: RLI and Byrne use different reading and memory instruments, different follow-up spans and study groups with different meanings. The Byrne source lineage was reconciled on 2026-08-16, so fresh outputs clear that input gate and record the checksum-bound source note. These remain adjusted or descriptive associations, not causal estimates, and input confirmation does not remove the design and comparability limits.
