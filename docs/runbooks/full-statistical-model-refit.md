...
| `level_factors`        | `factor_summary.csv` / `rope_summary.csv`                   | only the t2 change `d_grp_time[t2]` (role `causal`; `b_grp_time[1]` on a pre-#552 free-comparator fit) is randomised — a difference-in-differences of adjusted levels; `arm_gap_t1` is the t1 balance term (role `balance`, never an effect); `b_grp_time[t]` rows are the derived levels view (role `levels_view`) |
