...
| `did`                  | `did_summary.csv`                                           | `tau_t2` is the randomised causal contrast; `arm_gap_t1` checks balance; `arm_gap_t3` is the randomised early-start-versus-delayed-start schedule contrast and `delta_crossover = tau_t2 - arm_gap_t3` the change between the two regime contrasts, never an identified catch-up; every dose term is observational                                                                                                                                                                                      |
| `gain_factors`         | `treatment_marginal.csv`                                    | `trt_prob_*` (the only causal coefficient)                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `level_factors`        | `factor_summary.csv` / `rope_summary.csv`                   | only the t2 change `d_grp_time[t2]` (role `causal`; `b_grp_time[1]` on a pre-#552 free-comparator fit) is the randomised treated-versus-untreated effect — a difference-in-differences of adjusted levels; the `d_grp_time[t3]`/`[t4]` rows are randomised early-start-versus-delayed-start schedule contrasts (role `regime`), not adjusted associations; `arm_gap_t1` is the t1 balance term (role `balance`, never an effect); `b_grp_time[t]` rows are the derived levels view (role `levels_view`) |
