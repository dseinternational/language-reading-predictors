...
### Joint bivariate mechanism — `lrp-rli-jm-001` (`kind="joint_mechanism"`)

**Purpose.** Makes the letter-sound → word-reading review's **decoding-specificity** contrast an _identified_ posterior (#421 Tier 3; review note #424). It fits word reading (`W`) and nonword decoding (`N`) **jointly** on the same standardised letter-sound exposure, each with its own linear slope, while a **single** shared child random intercept (the best available latent-general-ability proxy) is partialled from both. The contrast Δ = β(LS→N) − β(LS→W) is then a within-model deterministic (`delta_ls_decoding`) — the true joint-posterior contrast rather than the product-of-marginals sensitivity that two separate mechanism fits (`mech-096` / `mech-101`) can only bound. Each outcome keeps its own autoregressive baseline and Beta-Binomial denominator (79 items for `W`, 6 for `N`), never pooled (a flattened-cell likelihood).

| Model            | Kind              | Outcome | Purpose                                                                                    |
| ---------------- | ----------------- | ------- | ------------------------------------------------------------------------------------------ |
| `lrp-rli-jm-001` | `joint_mechanism` | —       | identified decoding-specificity contrast: letter sounds → {word reading, nonword decoding} |

**Descriptive only.** Every slope is an adjusted association, latent general ability only partially proxied by the shared intercept — never causal. The Δ is a Campbell–Fiske convergent/discriminant argument for decoding-use, not identification of a causal decoding effect. The existing published contrast in `scripts/compare_statistical_models.py` (`tier1_decoding_specificity`, product-of-marginals) is left unchanged; rewiring it to prefer this model's identified Δ is a follow-up.
