> [!NOTE]
> Timing-scenario wording corrected by a LLM-based AI tool (Codex/GPT-6).
...
**Assessment timing.** Relative to the wait-list arm, the immediate arm's t2 assessment came about two weeks later (t1→t2 interval +0.44 months, t2→t3 −0.51 months, t1→t3 level), so every t2 contrast includes slightly more elapsed time in the immediate arm. The interval gap times each arm's t1→t2 gain per month gives two constant-rate scenarios, not bounds on timing bias. They assume the whole-interval rate applies during the extra weeks. The word-reading scenarios are 0.14 to 0.31 words beside a stored effect of 2.37 words. This comparison of point estimates cannot establish unchanged posterior conclusions. State the scenarios and their assumptions beside a t2 headline. See `METHODS.md` (**Assessment intervals**) and `notes/202609212100-assessment-interval-lengths.md`.
...
### Waitlist-crossover arm-by-wave sensitivity — `lrp-rli-did-001–lrp-rli-did-015` (+ `lrp-rli-did-101`–`lrp-rli-did-107`) (`kind="did"`)

**Purpose.** A longitudinal sensitivity analysis alongside the available-case modified ITT estimates. The binary-treatment models jointly model bounded t1/t2/t3 levels with a separate immediate-minus-waitlist gap at each wave.

**The three gaps, and what identifies each.** `arm_gap_t1` checks baseline balance and is never an effect. `tau_t2` is the randomised contrast of **assignment to immediate treatment versus no treatment yet**; it is the covariate-adjusted t2 arm-gap _level_, not the differenced `tau_t2 - arm_gap_t1`, so its baseline adjustment is soft and prior-weighted — a realised t1 imbalance is split between the tightly regularised `arm_gap_t1` and the shared child intercepts, and `lrp-rli-did-104` is the registered sensitivity over that split (2026-08-24 estimand sign-off, `notes/202608241100-did-t2-estimand-signoff.md`). `arm_gap_t3` is **also identified by the original randomisation** — assignment does not stop being random after crossover — but it contrasts a _different exposure_: the early-start treatment history against the delayed-start one, both arms having been treated by t3. It is therefore not a treated-versus-untreated effect, and it is not latent-ability-confounded; what it cannot supply is the mechanism, because duration, carryover, maturation, ceiling effects and different taught blocks are inseparable within it. `delta_crossover = tau_t2 - arm_gap_t3` is the change between those two randomised regime contrasts and is never evidence of an identified catch-up process (#576 finding 3).

A child random intercept partially pools stable between-child differences but does not make every child a fixed-effect control; `did_within_child_ppc.csv` tests the restrictive repeated-measures covariance that assumption implies. No model conditions on each period's start outcome: t2 is already treatment-affected for the immediate arm when used as the P2 baseline. The heavily floored outcomes (`P`, `N`) use a Bernoulli on wave-specific off-floor status, so their contrasts concern off-floor **prevalence**, not coming off the floor. Phoneme blending (`B`) carries the family's own mandatory response-link pairing: `lrp-rli-did-003` (ordinary logit) and `lrp-rli-did-103` (three-choice guessing floor) release only together.

Dose variants retain the P1/P2 transition frame, adjust for randomised arm, the arm-by-period cell structure, t1 outcome and t1 age, and estimate observational treated-centred session-dose associations. Because `treated = (immediate arm) OR (period 2)` saturates the four-cell design, `theta_treated` at the mean treated dose is a crossover _cell_ contrast rather than an isolated treatment-presence effect. Each dose fit publishes one named focal estimand — the natural-scale treated-row dose marginal in `dose_marginal_summary.csv` — which its prior pushforward, prior-sensitivity sweep and release gate all check.
...
child. The cohort contrast is **not** randomised (confounded by age-at-onset, timing and
window length — the immediate arm's window averages 15.5 months against the wait-list's 14.2,
worth about 0.7 words of the word-reading contrast), so _no_ term is causal — every coefficient
is an association. Design note:
...
| `lrp-rli-mech-306–311`        | as `196–201`      | Composite-ability mirror of the ability-adjusted panel: identical rows and terms, with the two-subtest `objass_c` composite (Block Design + Object Assembly) in place of `blocks` alone. The subtests correlate at 0.664, so one is ~0.66-reliable for what they share and the sum ~0.80-reliable; these fits answer "is the ability adjustment just one noisy subtest?". What the two share is a narrow visuospatial factor, **not** the latent `GA` — still adjusted associations |
| `lrp-rli-mech-312`            | `L → W`           | Composite-ability counterpart of the `258` HSGP curve, so the family's headline interquartile contrast gets the same reliability check                                                                                                                                                                                                                                                                                                                                              |

The table above is a selection; the family currently has 53 registered models (the curve tests `156–158` / `188–191`, the joint-readiness comparators `093–095` and the remaining companions are catalogued in `definitions.MODEL_REGISTRY` and the family findings note).
...
| `lrp-rli-med-387` | MED-087 under the three-choice guessing-floor outcome link (legacy `lrp87f`, #619)                                                                                                                  |

> **Phoneme blending carries a mandatory response-link pair (#619).** `lrp-rli-med-087` fits the ordinary inverse-logit **outcome** mean; **`lrp-rli-med-387`** refits it under the one-in-three guessing floor, because blending's ten items are three-alternative forced choice and an expected score cannot fall below chance. The ordinary fit carries the **largest** below-chance share of any registered `B` fit — 12.1 % of its posterior mass, above LRPITT08's 8.9 %. Neither half publishes without the other. Unlike every other family, the link here enters the **counterfactual simulation** rather than a summary: each NDE/NIE/total is a difference of simulated outcome means, so `score_mean_link` is threaded into `decompose`'s `outcome_p` and every cell is accumulated on the response scale. It governs the outcome only — a mediator is a separate leg with its own measure. Scope is the model of record: `med-187` is a declared interventional relabelling companion and is exempt.

### Predictor / within-child dynamics — `lrp-rli-adj-065`, `lrp-rli-lcsm-067/167/081/082/091`, `lrp-rli-dose-077` (+ variants)
...
| `lrp-rli-lcsm-067` | `lcsm`          | Within-child latent change-score: prior-wave letter sounds `L` and vocabulary `E` as predictors of reading _change_; one change intercept pooled over three unequal transitions                                                     |
| `lrp-rli-lcsm-167` | `lcsm`          | `lcsm-067` with arm × window change intercepts, absorbing the unequal transition lengths (about 7.1, 8.4 and 5.4 months) and the crossover schedule; compare its couplings with `067`'s                                             |
...
| `lrp-rli-dose-084` | `dose_response` | The same attendance-response on phoneme blending (`B`); link-paired with `lrp-rli-dose-384` (#619) and released only beside it                                                                                                      |
| `lrp-rli-dose-384` | `dose_response` | `dose-084` under the three-choice guessing-floor score-mean link (legacy `lrp84f`, #619)                                                                                                                                            |

> **Phoneme blending carries a mandatory response-link pair (#619).** `lrp-rli-dose-084` fits the ordinary inverse-logit score mean; **`lrp-rli-dose-384`** refits it under the one-in-three guessing floor, because blending's ten items are three-alternative forced choice and an expected score cannot fall below chance. The ordinary fit puts 7.0 % of its posterior mass below that floor. Neither half publishes without the other. This is the family #608 used to settle that observational families are **not** exempt: `METHODS.md` defines every dose fit's focal estimand as the natural-scale treated-row dose marginal, published in items, so it inherits the link exactly as a randomised contrast does.

The `dose_response` family reports the **intensive margin**: among children who were receiving the intervention, did attending more sessions go with more progress? Sessions enter centred and standardised over the on-intervention rows only, a separate `theta_treated` indicator carries the extensive margin (whether a child was being taught at all), and the exposure is split Mundlak-style into each child's study-average attendance and their within-child deviation from it — a single slope over a child random intercept returns a precision-weighted **blend** of the two, so calling it a within-child association, as this catalogue previously did, was wrong. Attendance is not randomised and the DAG has `A`, latent `GA` and `IG` all pointing into `IS`, so every dose slope is an adjusted association, never "more sessions cause more gain". The one randomised quantity in these fits is `theta_treated` read in period 1, where every immediate-arm child attended and every waiting control attended none. Otherwise only the available-case modified ITT estimates and the randomised-window DiD contrasts carry a causal interpretation under their stated selection and missing-data assumptions — in the lagged suite that is solely the window-1 arm contrast of `lcsm-081/082/167` (`itt_window1_contrast.csv`), reported as an available-case modified ITT consistency check.
