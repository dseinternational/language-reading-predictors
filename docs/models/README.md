...
**Assessment timing.** Relative to the wait-list arm, the immediate arm's t2 assessment came about two weeks later (t1→t2 interval +0.44 months, t2→t3 −0.51 months, t1→t3 level), so every t2 contrast includes slightly more elapsed time in the immediate arm. The first-order bound — the interval gap times each arm's t1→t2 gain per month — is 0.14 to 0.31 words beside word reading's 2.37-word effect and at most 8–16% of the stored effect for the other graded outcomes, except receptive and expressive vocabulary, where it is as large as their inconclusive effects. State it beside a t2 headline. See `METHODS.md` (**Assessment intervals**) and `notes/202609212100-assessment-interval-lengths.md`.
