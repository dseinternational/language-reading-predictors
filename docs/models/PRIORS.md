...
The Beta-Binomial is a **working likelihood for bounded total scores**. It enforces the observed score range and allows extra-binomial dispersion through `kappa`, but it does not establish that items with different difficulty—or scores produced under stopping rules—are exchangeable Bernoulli trials. Prior pushforwards and posterior-predictive checks must therefore include zero and ceiling mass, dispersion and the full score shape by arm and relevant baseline strata; a material mismatch calls for likelihood sensitivity rather than merely retuning `kappa`.
...
Not allowed as prospectively specified primary prior information:
...
The randomised treatment-effect prior `tau` is **tiered by outcome**: proximal (directly-taught / decoding) outcomes keep `Normal(0, 0.5)`; the distal broad-transfer set `measures.DISTAL_OUTCOMES = {R, E, T, F, UR, UE}` takes the tighter `Normal(0, 0.3)` (the `tau_distal` constructor). A single logit prior is not scale-invariant—`Normal(0, 0.5)` implies a plausible ~4-item swing on letter sounds but a ~35-item 95% swing on the 170-item ROWPVT/EOWPVT, exactly where large effects are least expected. The tier resolves from the outcome symbol in `factories._tau_sigma_for` and applies to the ITT `tau`, gain-factors `beta_trt`, level-factors group contrast and DiD `delta`—never to the adjusted-association group terms (mechanism `beta_G`, aligned `beta_cohort`). The joint model keeps the common `Normal(0, 0.5)`.

**Timeline disclosure.** The distal tier was adopted after the complete available-case modified ITT posterior table already existed. It is therefore a post-data, scale-calibrated regularisation decision, not a prior specified blind to this trial. The audit script `scripts/tau_prior_sensitivity.py` generates the common-`tau` sweep, `gamma_own ~ Normal(1, 0.25)` versus `Normal(1, 0.5)`, an unadjusted randomised-arm benchmark, and `kappa ~ HalfNormal(25/50/100/200)` concentration sensitivities for L/W. The reporting-config audit recorded in `notes/202607151648-itt-audit-reporting-refit.md` found distal R/E near null and proximal L/W directionally stable across the tested treatment-prior scales; the baseline and concentration axes changed the adjusted L/W estimates little, while W's unadjusted benchmark was materially less precise. Sensitivity stability does not make the original choice prospective. See `notes/202607011600-issue-141-prior-audit.md` §5a for the decision timeline.

**Floored-outcome conflict gate.** P/N use a different estimand and likelihood, so their prior check must not silently reuse the graded Beta-Binomial sweep. If power scaling diagnoses a possible `tau` prior-data conflict, `scripts/tau_prior_sensitivity.py --outcomes P N` refits the observed-baseline-floor subgroup with the Bernoulli off-floor likelihood under treatment-prior SD 0.5, 1.0 and 1.5, each with linear age on and off. The report is not ready for scientific interpretation until all six cells per outcome pass the all-free-variable convergence gate and retain a content-addressed trace. The release evaluator rejects swapped cells or self-declared CSV results by checking each trace's embedded cell/analysis/sampling provenance and recomputing convergence, risk differences and posterior probabilities from the draws. The report then shows the complete risk-difference and probability-of-benefit ranges rather than declaring a binary sensitivity “pass”; a wide range is evidence of prior dependence even though the computational gate is complete.

**Precision-prior caution.** A precision covariate is unnecessary for causal identification, but its model and prior are not incapable of changing `tau`. In a finite Bayesian sample, baseline imbalance, misspecified functional form or an overly concentrated baseline prior can move the treatment posterior. Test–retest correlation alone also does not identify a baseline regression slope: the slope depends on the pre/post standard deviations, measurement error and the link scale. Report the unadjusted randomised contrast and sensitivity to plausible `gamma_own` priors alongside the adjusted model.
...
| `precision`        | Prior on a pre-treatment covariate unnecessary for identification but included to sharpen an estimate; its specification can still affect the finite-sample posterior. |
...
| `tau`                     | `Normal(0, 0.5)`                                 | `causal` by default; sometimes `association` by override | `tau`, `beta_G`, `beta_period`, `delta`, `beta_trt`, `b_grp_time`, `d_grp_time`, `beta_grp`, `a_G`, `b_G`; `beta_cohort` and dose terms by contextual override   | Centred at no effect. A 95% Normal range is about +/-0.98 logits, or odds ratios about 0.38 to 2.66. Intended to be weakly informative on the logit scale while avoiding very large item-scale effects by default.                                             | Highest-impact review item. Run sensitivity with SD in `{0.25, 0.5, 0.75}` and push forward to probability and item scales.                                 |
| `gamma_own`               | `Normal(1, 0.25)`                                | `precision`                                              | `gamma_own`, `a_L`, `a_comp`, `b_W`                                                                                                                              | Encodes strong baseline continuity on the logit scale. The mean and narrow spread are informative; test–retest correlation does not by itself imply a slope of one, and this precision prior can influence `tau` in a finite imbalanced or misspecified model. | Compare an unadjusted arm model and wider or differently centred `gamma_own` priors in representative models, especially for noisy or floored baselines.    |
| `gamma_cross`             | `Normal(0, 0.3)`                                 | `association`                                            | `gamma_*`, `b_*`, `a_*`, interaction terms, ability terms, dose-stage terms, the `arm_gap_t1` balance term of the `did` and t1-referenced `level_factors` models | Shrinks cross-skill, covariate, interaction, and adjusted-association terms toward zero. A 95% Normal range is about +/-0.59 logits, or odds ratios about 0.55 to 1.80.                                                                                        | Run sensitivity with SD in `{0.2, 0.3, 0.5}` for high-impact adjusted couplings and interactions.                                                           |
...
| `joint_residual_corr`     | `LKJCholeskyCov(eta=4, sd_dist=HalfNormal(0.5))` | `nuisance`, `sensitivity-only`                           | `chol`, `corr`, `sigmas`, `z_raw` in the optional joint residual-correlation branch                                                                              | Optional residual-correlation block. It is off in every registered joint specification, so those graphs factorise by outcome and do not propagate within-child residual covariance into cross-outcome contrasts.                                               | If reactivated, document the LKJ shape and residual scales and require convergence plus paired probability-scale contrast checks.                           |
...
| `Normal(1, 0.25)` |              +0.51 to +1.49 |                 1.67 to 4.44 | Current informative own-baseline continuity prior; needs slope and baseline-noise sensitivity.      |
...
| Level factors | `lrp-rli-lf-001`                                                             | Clean t2 group contrast versus the later randomised schedule contrasts (role `regime`) and the adjusted associations. |
