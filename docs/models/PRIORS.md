...
**Timeline disclosure.** The distal tier was adopted after the full ITT posterior table already existed. It is therefore a post-data, scale-calibrated regularisation decision, not a prior specified blind to this trial. The audit script `scripts/tau_prior_sensitivity.py` now generates the common-`tau` sweep, `gamma_own ~ Normal(1, 0.25)` versus `Normal(1, 0.5)`, an unadjusted randomised-arm benchmark, and `kappa ~ HalfNormal(25/50/100/200)` concentration sensitivities for L/W. Earlier reviewed results show distal R/E remain near null and proximal L/W retain their direction across the tested treatment-prior scales, but sensitivity stability does not make the choice prospective; the newly added baseline and concentration axes require fresh reporting fits before interpretation. See `notes/202607011600-issue-141-prior-audit.md` §5a for the timeline and prior numerical results.
