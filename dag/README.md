...
| `dag-reading-language-memory.dagitty`               | Adopted working contemporaneous graph for the observational Byrne/RLM cohort (`study_id="rlm"`); arrows are causal hypotheses, but the observational cohort does not identify them as effects.                                                                                   |
...
The graph is approved as a structural working hypothesis only. A reciprocal model requires a simulation feasibility check before registration; `notes/202608141700-byrne-lagged-dag-decision.md` requires a simulation gate over the real missingness patterns and distinguishes the paper's waves 1–3 from later prepared-data sensitivities.
