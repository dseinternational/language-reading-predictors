...
Single-outcome ITT modules declare immutable `IttModelSettings` rather than free-form dictionary keys. `resolve_itt_run_plan()` checks for unknown or contradictory settings before an output directory is reset or data are loaded, then supplies the same concrete `IttRunPlan` to preprocessing, the PyMC factory, diagnostics, metadata and influence refits. Each ITT fit records that plan under `resolved_run_plan` in `config.json` and writes `model_recipe.md`, a plain-language account of the question, analysis rows, likelihood, causal treatment term, associational precision terms and required checks.

Step 2 of the methodology: Bayesian models fit with PyMC. One module per model — `lrp_rli_itt_NNN.py` (the DAG-faithful ITT suite + companions), `lrp_rli_did_NNN.py` (the waitlist-crossover / DiD family), `lrp_rli_gf_NNN.py` / `lrp_rli_lf_NNN.py` (the DAG-focused gain- and level-factor families), `lrp_rli_al_NNN.py` (the aligned-40-week per-protocol family) and the bare-family modules `lrp_rli_mech_NNN.py` / `lrp_rli_med_NNN.py` (mechanism/mediation models) — each defining a `SPEC = ModelSpec(...)` and a `fit(config)` that calls the matching pipeline entry point. Families are keyed by `ModelSpec.kind`, with a factory in `factories.py` and a pipeline in `pipeline.py`; `definitions.KINDS` is the authoritative family list. The eight detailed below are joined by `adjusted`, `corr_factor`, `dose_response`, `lcsm`, `mediation_multi`, `horseshoe`, `growth`, `historical_growth`, `historical_joint`, `survival`, `block_exposure`, `concurrent` and `long_corr_factor`. The exact model-id ranges in the per-family bullets below can go stale as models are added — `docs/models/README.md` and `definitions.MODEL_REGISTRY` are the authoritative catalogue:
