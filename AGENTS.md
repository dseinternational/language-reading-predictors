...
Step 2 of the methodology: Bayesian models fit with PyMC. One module per model — `lrp_rli_itt_NNN.py` (the DAG-faithful ITT suite + companions), `lrp_rli_did_NNN.py` (the waitlist-crossover / DiD family), `lrp_rli_gf_NNN.py` / `lrp_rli_lf_NNN.py` (the DAG-focused gain- and level-factor families), `lrp_rli_al_NNN.py` (the aligned-40-week per-protocol family) and the bare-family modules `lrp_rli_mech_NNN.py` / `lrp_rli_med_NNN.py` (mechanism/mediation models) — each defining a `SPEC = ModelSpec(...)` and a `fit(config)` that calls the matching pipeline entry point. Families are keyed by `ModelSpec.kind`; `definitions.KINDS` is the authoritative family list. `stages.py` owns the shared attach, sampling, diagnostics, posterior-predictive, metadata and report-finalisation order, plus the invariant primary-fit lifecycle (`PrimaryFitPlan` + `run_primary_fit`, #394 — prior predictive → sampling/LOO → summary diagnostics + psense → posterior predictive → convergence gate + extended diagnostics → trace persistence; `fit_survival` is the reference adoption). Family orchestration is moving out of the `pipeline.py` monolith into `pipelines/` a group at a time (#394 steps 5–6). Fourteen families are there now — `itt` (with the floor branch) and `joint`, then `did`, `dose_response`, `gain_factors`, `level_factors`, `block_exposure` and `aligned`, then `mechanism`, `joint_mechanism` and `mediation` (which owns three fit entry points plus `prepare_mediation_data`), then `adjusted`, `horseshoe` (each carrying the Byrne/RLM port of the same family as a second entry point) and `concurrent` — and `pipeline.py` re-exports every entry point so model modules and tests keep their import path. Only survival, `lcsm`, growth, historical growth and the Byrne measurement/joint-growth fits are left in the monolith. The shared layer beneath them — `runtime.py` (the `StageHooks` binding and `require_spec`), `publication.py` (banners, report-template copy, model graph), `adjustment.py` (the fitted adjustment-set record), `prior_artifacts.py`, `ppc_artifacts.py` and `figure_artifacts.py`, plus `diagnostics.sample_subfit` (the sub-fit sampler, beside the primary sampler it mirrors) and the posterior summaries in `reporting.py` — must never import `pipeline.py`, or the cycle that kept every family in the monolith returns; `tests/statistical_models/test_pipeline_boundaries.py` enforces that, and its `MIGRATED_FAMILIES` map must be updated whenever a family moves. ITT is the reference family-owned implementation; the families still awaiting a typed run plan (see above) retain the `factories.py` and `pipeline.py` compatibility boundaries during the incremental Phase 3 migration — `mediation` is the outlier, with three fit functions. The eight detailed below are joined by `adjusted`, `corr_factor`, `dose_response`, `lcsm`, `mediation_multi`, `horseshoe`, `growth`, `historical_growth`, `historical_joint`, `joint_mechanism`, `survival`, `block_exposure`, `concurrent` and `long_corr_factor`. The exact model-id ranges in the per-family bullets below can go stale as models are added — `docs/models/README.md` and `definitions.MODEL_REGISTRY` are the authoritative catalogue:
