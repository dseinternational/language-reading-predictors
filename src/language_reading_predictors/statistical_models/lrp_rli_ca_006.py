...
"""LRP-CA-006 - Concurrent conditional associations: skills -> expressive
vocabulary, per wave.

#336 extends the concurrent conditional-associations family to standardised expressive
vocabulary (E; EOWPVT) as the focal outcome. At each timepoint it fits a between-child
Beta-Binomial regression of E's *level* on the standardised same-wave logits of the
other core skills (word reading W, letter sounds L, blending B, taught receptive TR
and expressive TE vocabulary, and standardised receptive R vocabulary), plus age and
...
of the same measure set. Floored measures (P, N) stay excluded. E is a 170-item
...
SPEC = ModelSpec(
    model_id="lrp-rli-ca-006",
    kind="concurrent",
    title=("Concurrent conditional associations: concurrent skills -> expressive vocabulary (per wave)"),
    outcome_symbol="E",
    family="concurrent",
    ...
    model_settings=ConcurrentModelSettings(
        # The core skill set minus the focal (E). Floored P/N are excluded as
        # predictors (issues #312 and #336).
        predictor_symbols=("W", "L", "B", "TR", "TE", "R"),
