...
**Bernoulli on the binary off-floor indicator** (period post > 0): `beta_trt` is
the off-floor log-odds (its items-scale marginal collapses to the off-floor risk
difference). DAG-focused factors: on-intervention (causal), own baseline, age,
ability (blocks), and the upstream skills letter sounds (L) + blending (B); SES
excluded (non-DAG / redundant). Flagged off-floor in the report.
"""
...
SPEC = ModelSpec(
    ...
    kind="gain_factors",
    title="Gain factors for phonetic spelling (P), off-floor",
    outcome_symbol="P",
    extra={
        "skill_symbols": ("L", "B"),
