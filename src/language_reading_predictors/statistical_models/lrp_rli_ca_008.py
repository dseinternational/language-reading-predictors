...
"""LRP-CA-008 - Concurrent conditional associations: skills -> basic concepts, per wave.

Extends the concurrent family beyond its original reading/vocabulary core set to
Basic concepts (CELF) (F), a scope addition (#371) so the levels panel of the association
matrix covers all non-floored outcomes. At each timepoint it fits a between-child
Beta-Binomial regression of basic concepts's *level* on the standardised same-wave logits
...
F is not floored, so it enters as a focal outcome on the same footing as the
...
SPEC = ModelSpec(
    model_id="lrp-rli-ca-008",
    kind="concurrent",
    title="Concurrent conditional associations: concurrent skills -> basic concepts (per wave)",
    outcome_symbol="F",
    family="concurrent",
