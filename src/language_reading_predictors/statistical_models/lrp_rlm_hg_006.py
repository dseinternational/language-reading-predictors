...
"""RLMHG06 - historical BAS recall-of-digits growth, waves 1-4 + DS wave 5 (Byrne et al., #164/#338).

A descriptive group-by-wave growth model for BAS recall of digits (``basdig``, a
verbal short-term / phonological-memory measure) over waves 1-4 (plus the Down-syndrome-only wave-5 tail) of
the Byrne, MacDonald & Buckley reading-language-memory study (``study_id="rlm"``),
the per-measure sibling of ``lrp-rlm-hg-001`` (word reading). It runs through the
shared historical-growth pipeline (:func:`pipelines.historical_growth.fit_historical_growth`), so it
uses the same sampler, convergence gate, output layout and report conventions.
...
**Confirmed ceiling (#338).** The Beta-Binomial denominator (``n_trials=34``)
is the instrument's confirmed maximum (``n_trials_confirmed=True`` in
``datasets.RLM_MEASURES``): BAS Recall of Digits has 34 items (CLOSER cognitive-measures guide; Parsons, 2014); the cohort reaches this ceiling. Researched and signed off by the data owner
...
SPEC = ModelSpec(
    ...
    title="Historical BAS recall-of-digits growth, waves 1-4 + DS wave 5 (Byrne et al.)",
