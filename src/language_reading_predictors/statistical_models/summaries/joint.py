...
def tau_difference_summary(
    ...
    *,
    ...
    result: dict[str, float | str] = {
        ...
        "dependence_moments_method": "paired_ame_draws_v1",
        "left_ame_prob_variance": float(np.var(ame[ia], ddof=1)),
        "right_ame_prob_variance": float(np.var(ame[ib], ddof=1)),
        "ame_prob_covariance": float(np.cov(ame[ia].ravel(), ame[ib].ravel(), ddof=1)[0, 1]),
        "diff_prob_variance": float(np.var(diff_prob, ddof=1)),
