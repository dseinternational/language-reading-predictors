...
"""LRPDID05 - waitlist-crossover / difference-in-differences ITT effect on receptive vocabulary (ROWPVT) (R).

A within-person replication of the randomised ITT effect (LRPITT05). Stacks the two
...
DiD). Compare delta against the single-outcome RCT tau (LRPITT05).
...
SPEC = ModelSpec(
    ...
    kind="did",
    title="Waitlist-crossover (DiD) ITT effect of the intervention on receptive vocabulary (ROWPVT) (R)",
    outcome_symbol="R",
    extra={
        "outcomes": ("R",),
        "periods": (0, 1),
