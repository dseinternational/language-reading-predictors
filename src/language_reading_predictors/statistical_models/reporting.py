...
def ppc_coverage_markdown(cov: pd.DataFrame) -> str:
    ...
    report (#271). Returns an empty string for an empty/None frame so the partial can
    ``print`` it unconditionally.
    """
    if cov is None or len(cov) == 0:
        return ""
    d = cov.drop_duplicates("level_pct").set_index("level_pct")
    unit = str(cov["unit"].iloc[0])
    quantity = str(cov["quantity"].iloc[0])
    clauses: list[str] = []
    if 90 in d.index:
        r90 = d.loc[90]
        clauses.append(
