...
Sign note: ``tau`` is the coefficient on the waiting-control indicator
(``G = group - 1``; group 2 waits for intervention), so a *negative* ``tau`` means
the intervention raised that outcome. The reported contrast is therefore
``tau[UE] - tau[TE]`` (= benefit on taught minus benefit on not-taught): a
*positive* value means the programme moved the words it taught *more* than
...
SPEC = ModelSpec(
    ...
    # sensitivity note if it destabilises sampling. ``difference=("UE","TE")``
    # asks the pipeline to summarise tau[UE] - tau[TE] (benefit on taught minus
