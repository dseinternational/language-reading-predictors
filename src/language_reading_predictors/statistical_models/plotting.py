...
"""Shared figure-saving helpers for the statistical models.

These centralise the report figure-artifact policy (issue #208) so every
plotting call site behaves identically:

* always write ``<name>.png`` — the artifact the report templates reference
  (raster keeps the model-output pages quick to browse);
* also write ``<name>.svg`` unless it would exceed ~2 MB, in which case the
  vector sibling is dropped (very large SVGs are the ones that make the viewer
  slow — exactly what #208 wants to avoid);
* optionally write ``<name>.csv`` of the data behind the plot.

Consistent house style (fonts, colours, grid, DPI) comes from
``dse_research_utils`` (``set_matplotlib_default_style``), applied once at the
fit entry points; these helpers only standardise *saving* — PNG + SVG sibling +
optional data CSV — and closing the figure. Both matplotlib figures and
``arviz_plots`` ``PlotCollection`` objects route through here so a single change
propagates to every model.
"""
...
# report viewer slow). ~2 MB is comfortably above a typical vector figure and
# well below the multi-megabyte beeswarm/interaction grids we want to keep raster.
...
def _write_svg_sibling(save, base: str, svg_max_bytes: int = SVG_MAX_BYTES) -> None:
    ...
    works for both matplotlib figures and ``arviz_plots`` collections. Guarded so
    an SVG-backend hiccup never costs us the (already-written) PNG.
    """
    svg = base + ".svg"
    try:
        save(svg)
