...
def configure_data_categories(df: pd.DataFrame):
    ...
    df[vars.GROUP] = df[vars.GROUP].map(cats.TIME).astype("category")
    ...
    #    df[vars.REPEATEDOM] = df[vars.REPEATEDOM].map(cats.NO_YES).astype("category")
    df[vars.EARINF] = df[vars.EARINF].map(cats.NO_YES).astype("category")
    #    df[vars.GROMMETS] = df[vars.GROMMETS].map(cats.NO_YES).astype("category")
    #    df[vars.GLASSES] = df[vars.GLASSES].map(cats.NO_YES).astype("category")
