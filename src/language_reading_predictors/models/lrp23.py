...
"""
LRP23: Predictors of taught-vocabulary gains.

``LRP23`` is the exploratory model for *taught* expressive-vocabulary gains
(``b1extau_gain`` — change in the Block 1 directly-taught expressive vocabulary
score). It is the taught-vocabulary analogue of :mod:`lrp03` (standardised
...
outlier exclusion) used by LRP03 carries over.
...
substantive predictors. This is the only deviation from the LRP03 predictor set.

Status: initial exploratory baseline. Hyperparameters are borrowed from the
LRP03 standardised analogue as a reasonable starting point; a target-specific
Optuna tune (``scripts/tune_model.py lrp23``) and iterative importance-based
feature selection are follow-ups, exactly as for LRP03.
"""
...
# ── hyperparameter set ───────────────────────────────────────────────────
#
# Borrowed from LRP03 (MAE-tuned on eowpvt_gain) as a starting baseline for the
# closely-matched taught-vocabulary gain target. Retune for this target before
# any quantitative reporting (scripts/tune_model.py lrp23); importance rankings
...
class LRP23(GainModel):
    ...
    notes = (
        ...
        "leakage. Hyperparameters borrowed from lrp03 pending a target-specific "
