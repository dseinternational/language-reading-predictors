...
# MAE-tuned on the 4-predictor replication-selected set, no outlier
# exclusion (Optuna 150 trials, 10-split GroupKFold, seed 47, scoring=mae,
# lgbm_objective=mae). Tuner-inner CV MAE 2.3251. Supersedes the full-set tune.
...
class LRP18(LevelModel):
    ...
    notes = (
        "Exploratory model for aptgram (level). Feature-selected (2026-06-20 replication) from the full 32-predictor default set to 4 predictors via a distance-correlation redundancy filter (no dcor >= 0.70 pairs remain) plus an importance noise-floor cut, then re-tuned on the reduced set (tuner-inner CV MAE 2.535 -> 2.325). Only the dominant predictor is robustly above the importance noise floor; treat the reduced ranking as exploratory. See the SelectionStep and notes/202606201500-gb-replication-findings.md."
    )
