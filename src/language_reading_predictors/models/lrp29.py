...
class LRP29(GainModel):
    ...
    notes = (
        "Exploratory model for erbto_gain (gain). Uniform feature selection (2026-06-23) from the full 35-predictor DEFAULT_GAIN set to 10 predictors (distance-correlation redundancy filter + importance noise-floor cut; baseline force-kept; no dcor >= 0.70 pairs remain), re-tuned on the reduced set (tuner-inner CV MAE 3.548). Gain models are near-noise (baseline-driven regression to the mean) — treat the reduced ranking as exploratory. See notes/202606230900-predictability-speech-memory-language.md."
    )
