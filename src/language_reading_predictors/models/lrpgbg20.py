...
class LRPGBG20(GainModel):
    ...
    notes = (
        "Exploratory model for deappin_gain (gain). Uniform feature selection (2026-06-23) from the full 34-predictor DEFAULT_GAIN set to 9 predictors (distance-correlation redundancy filter + importance noise-floor cut; baseline force-kept; no dcor >= 0.70 pairs remain), re-tuned on the reduced set (tuner-inner CV MAE 4.738). Gain models are near-noise (baseline-driven regression to the mean) — treat the reduced ranking as exploratory."
    )
