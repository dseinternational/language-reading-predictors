...
# correlation from cross-domain signal. Pooled OOF R² 0.62 -> 0.40.


class LRP32NoConstruct(LRP32):
    """deappin — same-skill variant: DEAP sibling(s) dropped (R2 0.62 -> 0.40)."""

    model_id = "lrp32_noconstruct"
    variant_of = "lrp32"
    description = (
        "LightGBM — deappin predictors "
    ...
    selection_steps = [
        SelectionStep(
            ...
            notes=(
                "Same-skill (concurrent) variant of lrp32: drops the same-instrument DEAP sibling(s) ``deappvo``, ``deappfi`` (scored from the same instrument as the target). Pooled out-of-fold R² falls from 0.62 to 0.40 -> retains substantial cross-domain signal — not purely measurement-bound. Reuses the primary's MAE-tuned params on the reduced set (the sibling-dropped OOF R² is the headline; re-tuning is a refinement). See notes/202606230900-predictability-speech-memory-language.md."
            ),
            date="2026-06-23",
            metrics_before={"pooled_oof_r2": 0.6226},
            metrics_after={"pooled_oof_r2": 0.3993},
        ),
    ]
    notes = (
        "Same-skill (concurrent) variant of lrp32: drops the same-instrument DEAP sibling(s) ``deappvo``, ``deappfi`` (scored from the same instrument as the target). Pooled out-of-fold R² falls from 0.62 to 0.40 -> retains substantial cross-domain signal — not purely measurement-bound. Reuses the primary's MAE-tuned params on the reduced set (the sibling-dropped OOF R² is the headline; re-tuning is a refinement). See notes/202606230900-predictability-speech-memory-language.md."
