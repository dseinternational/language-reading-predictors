...
def test_gate_exception_releases_a_model_into_the_comparison(cmp_mod, out_root):
    """A recorded, signed-off gate exception (#412 machinery) must release a model
    into the comparison layer on exactly the checks it waives — the #438 gap where
    the exception was honoured by the report but not here, so an excepted model was
    silently dropped from its own nested LOO contrast."""
    _install_run(cmp_mod, "excepted", passed=False)
    run = Path(cmp_mod._run_dir("excepted", "dev"))
    (run / "diagnostics_summary.json").write_text(
        json.dumps(
            {
                "passed": False,
    ...
    (run / "config.json").write_text(
        json.dumps(
            {"spec_extra": {"gate_exception": {"checks": ["divergences"], "issue": 404}}}
        )
    )
    assert cmp_mod._gate_status("excepted", "dev") == "GATE_EXCEPTION"
    assert cmp_mod._gate_ok("excepted", "dev") is True


def test_gate_exception_does_not_waive_an_unnamed_failure(cmp_mod, out_root):
    """Waiving is per-check: a fit that also fails R-hat stays REVIEW even though the
    exception names divergences. The exception is evidence about one diagnostic, not
    a blanket release."""
    _install_run(cmp_mod, "excepted-rhat", passed=False)
    run = Path(cmp_mod._run_dir("excepted-rhat", "dev"))
    (run / "diagnostics_summary.json").write_text(
        json.dumps(
            {
                "passed": False,
    ...
    (run / "config.json").write_text(
        json.dumps({"spec_extra": {"gate_exception": {"checks": ["divergences"]}}})
    )
    assert cmp_mod._gate_status("excepted-rhat", "dev") == "REVIEW"
    assert cmp_mod._gate_ok("excepted-rhat", "dev") is False
