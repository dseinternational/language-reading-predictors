...
- The cross-layer comparisons (`scripts/rank_predictors.py`, `scripts/compare_horseshoe_vs_gb.py`, `scripts/compare_gb_vs_statistical.py`) were not rerun. They can now pair these fits with the 21 September statistical run root. `output/ranking/` still holds earlier output.
