...
The family has five models — three view **word reading (WR)**, one views **letter sounds (LS)**, one views **phoneme blending (PA)**:
...
| Model      | Outcome                  | Best estimate (items) | 89% credible range | P(>0) | Evidence    | Causal?     |
| ---------- | ------------------------ | --------------------- | ------------------ | ----- | ----------- | ----------- |
| `dose-277` | W (word reading), pooled | **+1.28**             | +0.57 to +2.04     | 0.998 | very strong | association |
| `dose-077` | W, period-resolved       | **+1.16**             | +0.38 to +1.98     | 0.992 | very strong | association |
| `dose-177` | W, ability-adjusted      | **+1.28**             | +0.47 to +2.12     | 0.994 | very strong | association |
| `dose-083` | L (letter sounds)        | **+0.80**             | +0.14 to +1.41     | 0.973 | strong      | association |
| `dose-084` | B (phoneme blending)     | **+0.29**             | −0.00 to +0.57     | 0.942 | moderate    | association |

The cleanest single number is the **pooled** slope (`dose-277`): a 1-SD (≈31-session) increase went with **+1.3 more words read**, 89% credibly +0.6 to +2.0, 99.8% positive — a very strong _association_. On the logit (model) scale this is `dose_pooled` = 0.143 (89% 0.065 to 0.224). Its period-resolved twin `dose-077` agrees (+1.16 items, very strong), and adjusting for general ability (`dose-177`) leaves it essentially unchanged (+1.28 items) — so the word-reading dose link is not merely a baseline-ability artefact, though it remains confounded. Letter sounds (`dose-083`) is intermediate (+0.80 items, strong), and phoneme blending (`dose-084`) is the weakest signal in the family (+0.29 items, moderate — its interval just crosses zero).
...
| Model      | Outcome        | Overall (p)               | Period 1 (p)      | Period 2 (p)      | Period 3 (p)  | σ between periods (89%) |
| ---------- | -------------- | ------------------------- | ----------------- | ----------------- | ------------- | ----------------------- |
| `dose-077` | W              | 0.137 (0.904, suggestive) | **0.177 (0.998)** | 0.135 (0.937)     | 0.097 (0.867) | 0.107 (0.010–0.444)     |
| `dose-177` | W, ability-adj | 0.149 (0.909, suggestive) | **0.199 (0.999)** | 0.150 (0.948)     | 0.100 (0.872) | 0.117 (0.012–0.460)     |
| `dose-083` | L              | 0.148 (0.846, suggestive) | **0.227 (0.995)** | 0.169 (0.899)     | 0.054 (0.648) | 0.180 (0.022–0.577)     |
| `dose-084` | B              | 0.138 (0.811, suggestive) | 0.104 (0.828)     | **0.291 (0.962)** | 0.037 (0.597) | 0.229 (0.026–0.663)     |
