...
| ------------ | ------------------------------------------------------ | -------------- | ----------------------------------- | ----- | ----------- |
| **mech-088** | Taught receptive vocab (TR) → W †                      | TR 6→23        | **+9.0 items** (+4.0 to +14.0)      | ~100% | very strong |
| **mech-089** | Taught expressive vocab (TE) → W †                     | TE 0→19        | +8.6 items (+1.9 to +14.4)          | 99.3% | very strong |
| **mech-073** | Letter sounds (L) → W, age-moderated                   | L 2→32         | +7.3 items (+2.2 to +12.7)          | 99.8% | very strong |
| **mech-158** | L → W, complete-case comparator                        | L 2→32         | +6.9 items (+1.5 to +12.4)          | 99.6% | very strong |
| **mech-058** | Letter sounds (L) → W                                  | L 2→32         | +6.7 items (+1.6 to +11.9)          | 99.6% | very strong |
| mech-173     | L → W (+age, no-interaction baseline)                  | L 2→32         | +6.6 items (+1.6 to +11.8)          | 99.6% | very strong |
| mech-057     | Expressive vocab (E) → W                               | E 10→77        | +5.3 items (−1.9 to +12.1)          | 92.7% | moderate    |
| mech-071     | L → W, moderated by expressive vocab                   | L 2→32         | +5.2 items (+0.2 to +10.9)          | 98.3% | strong      |
| **mech-072** | L (moderated by blending B) → **nonword decoding (N)** | L 2→32         | +4.0 items (+2.5 to +5.0)           | ~100% | very strong |
| mech-172     | L + B main effects → N (no-interaction baseline)       | L 2→32         | +3.3 items (+2.0 to +4.4)           | ~100% | very strong |
| mech-090     | Phonological memory (erbto) → W                        | 3→36 (erbto)   | +3.1 raw-score units (−0.5 to +6.6) | 95.2% | moderate    |
| mech-056     | Receptive vocab, std (R) → W                           | R 12→82        | +2.9 items (−3.8 to +9.4)           | 80.4% | suggestive  |
...
## Results — all models (with 90% ranges and adjusted-association detail)
...
| Model    | Exposure → outcome    | Best est. (median) | 95% credible range | 90% range     | P(>0)  | Evidence    | Causal?     |
| -------- | --------------------- | ------------------ | ------------------ | ------------- | ------ | ----------- | ----------- |
| mech-056 | R → W                 | +2.9 items         | −3.8 to +9.4       | −2.7 to +8.4  | 0.804  | suggestive  | association |
| mech-057 | E → W                 | +5.3 items         | −1.9 to +12.1      | −0.7 to +11.0 | 0.927  | moderate    | association |
| mech-058 | L → W                 | +6.7 items         | +1.6 to +11.9      | +2.5 to +11.0 | 0.996  | very strong | association |
| mech-071 | L → W (× E)           | +5.2 items         | +0.2 to +10.9      | +0.9 to +9.9  | 0.983  | strong      | association |
| mech-072 | L → N (× B)           | +4.0 items         | +2.5 to +5.0       | +2.8 to +4.8  | ~1.0   | very strong | association |
| mech-073 | L → W (× age)         | +7.3 items         | +2.2 to +12.7      | +3.0 to +11.8 | 0.998  | very strong | association |
| mech-088 | TR → W (adjusts IS) † | +9.0 items         | +4.0 to +14.0      | +4.8 to +13.2 | 0.9996 | very strong | association |
| mech-089 | TE → W (adjusts IS) † | +8.6 items         | +1.9 to +14.4      | +3.0 to +13.5 | 0.993  | very strong | association |
| mech-090 | erbto → W             | +3.1 raw units     | −0.5 to +6.6       | +0.04 to +6.1 | 0.952  | moderate    | association |
| mech-158 | L → W (complete-case) | +6.9 items         | +1.5 to +12.4      | +2.3 to +11.6 | 0.996  | very strong | association |
| mech-172 | L → N (+ B main)      | +3.3 items         | +2.0 to +4.4       | +2.2 to +4.3  | ~1.0   | very strong | association |
| mech-173 | L → W (+ age main)    | +6.6 items         | +1.6 to +11.8      | +2.4 to +11.0 | 0.996  | very strong | association |
...
- **mech-071 — L → W moderated by expressive vocabulary (E).** The E-moderator **main term** is +0.191 logit (95% +0.053 to +0.328, P(>0) = 0.997, very strong): among otherwise-alike children, higher expressive vocabulary is associated with more word reading. The **L × E interaction** is −0.065 (95% −0.173 to +0.038, P(>0) = 0.110) — i.e. unresolved: the data neither confirm nor rule out that expressive vocabulary flattens or steepens the letter-sound slope.
- **mech-072 — L → N moderated by blending (B), the code-route signature.** Blending **main term** +0.361 logit (95% +0.033 to +0.683, P(>0) = 0.985, strong). The **L × B interaction** is a clear **negative**: median −0.327 (95% −0.627 to −0.039, P(>0) = 0.013, i.e. P(<0) = 0.987, strong). Read plainly: higher blending skill **flattens** the letter-sound → nonword-decoding slope — the two code skills are partial substitutes rather than additive, so knowing more letter sounds buys less extra nonword decoding once a child already blends well.
- **mech-073 — L → W moderated by age.** The **age × L interaction** is −0.057 (95% −0.133 to +0.021, P(>0) = 0.072) and the **age main term** is −0.069 logit (95% −0.171 to +0.032, P(>0) = 0.091) — both point mildly negative but are unresolved.
- **mech-172 — L + B main effects → N (the no-interaction baseline for mech-072).** Blending **main term** +0.281 logit (95% −0.037 to +0.590, P(>0) = 0.958, moderate). With the interaction removed, blending still carries a positive additive association with nonword decoding.
- **mech-173 — L → W with an age main term (the no-interaction baseline for mech-073).** The **age main term** is −0.100 logit (95% −0.194 to −0.006, P(>0) = 0.019, i.e. P(<0) = 0.981, strong negative): holding letter sounds fixed, older children in this stacked cross-phase view are associated with slightly _less_ word reading — a between-child confounding pattern, not a within-child decline, and certainly not causal.
...
- **Knee** (where the curve steepens): median ≈ **29.5 of 32 letter-sound items**, credible range roughly 13 to 29.5 — near the top of the observed range.
