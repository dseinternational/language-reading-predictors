...
Break ② deserves a closer look at _how_ the 2019 model reaches "via phoneme awareness, specifically not letter-sound knowledge or RAN", because the determination is thinner than the wording implies. The three foundations enter as **parallel** mediators — each regressed simultaneously on speech + language + IQ (with the mediators' residuals free to correlate), and t2 reading regressed on all three plus t1 reading. An indirect route carries an effect only if _both_ legs are non-trivial: an _a_-path (speech predicts the mediator) and a _b_-path (the mediator predicts reading). The whole determination turns on the _a_-path. Letter-sound knowledge and RAN are **not** weak reading predictors — letter-sound knowledge is the single strongest concurrent correlate of reading in the sample (latent _r_ ≈ 0.79) — they fail to mediate only because **speech does not uniquely predict them** once language and IQ are partialled out (`speech → LSK` and `speech → RAN` are non-significant, while `speech → PA` survives). The zero-order correlations foreshadow it (speech ↔ PA = −0.21, ↔ LSK = −0.14, ↔ RAN = −0.07, ns), and two model-level checks close it: the model-implied indirect effect is significant only for the PA route, and dropping the direct `speech → reading` path costs no fit. So **"not via letter sounds" means "speech is not specifically tied to letter sounds", not "letter sounds do not matter for reading"**.

Four caveats make that specificity claim weaker than it reads — each an absence-of-evidence or collinearity issue rather than a positive result: (a) it rests on two _non-significant_ `a`-paths at low power (speech difficulty is 6.88 %, 38 children); (b) the mediators are collinear (PA ↔ LSK _r_ ≈ 0.5–0.7), so partitioning a shared speech signal between them is unstable — "loads on PA, not LSK" can flip with small changes in sample or measurement; (c) no test that the indirect effects _differ_ is reported, and a significant-vs-non-significant contrast is not a difference test (Gelman & Stern 2006); and (d) speech, PA, LSK and RAN are all measured at the same wave, so "travels through PA" is a decomposition of concurrent covariance, not a demonstrated sequence.

This sharpens break ② from an attenuation into an **inversion**. The TD story is "via phoneme awareness, specifically _not_ letter-sound knowledge." Our own DS mediation finds the opposite partition inside the same code route: the intervention's reading effect runs **via letter sounds, not vocabulary** (med-059/064), with blending (our PA proxy) weak — consistent with Cossu et al. (1993) and Hulme et al. (2012, _Dev Sci_) showing phoneme awareness is a poor reading-growth predictor in DS. So the mediator that carries the effect in TD (phoneme awareness) is the DS pathway that _doesn't_, and the mediator TD rules out (letter-sound knowledge) is the one that _does_ in DS. The transfer fails at the level of _which mediator_, not merely _how strong_ — the strongest possible form of "quarantine the mechanism".
...
### How severe is DS speech on the same instrument? A distributional argument for the flip

There is a further, quantitative reason the TD mediation result should be _adopted for orientation but not for mechanism_: the two samples do not share a speech-accuracy support, so the TD structure is being extrapolated off its estimation range. This can be shown directly because **both studies use the same instrument** — the DEAP articulation / picture-naming task (Dodd et al. 2002) — so a like-for-like comparison of consonant production is available. Burgoyne 2019 scores it as Percentage Consonants Correct (PCC) pooled over 43 consonants; our data (`deappin`/`deappvo`/`deappfi`, with `deappav` their mean and `deapp_c` their sum) score the same picture-naming task by word position. Because the DS vowel score is near-ceiling (mean 96 %), the fair PCC analog is the **consonants-only** mean (initial + final)/2, matching Burgoyne's consonant-only PCC and excluding the vowel that flatters the composite.

In Burgoyne 2019, "speech difficulty" was **PCC ≤ 85 %** — 1.5 SD below the TD sample mean (the Hayiou-Thomas et al. 2017 clinical cut) — and it flagged a rare tail: **38 children, 6.88 %** of the 569-child cohort at school entry (~5 yr). Within that flagged group, PCC was **mean 78.3 %, range 53.5–83.7 %** — so even the TD difficulty tail produces ~78 % of consonants correctly, and the single worst-affected child in the whole sample sat at ~53 %. Our DS cohort at t1 (n = 51 with DEAP, of 54; mean age **82 months ≈ 6.9 yr — about two years _older_ than Burgoyne's t1**) sits far below that:

| DEAP Picture Naming (% correct), DS t1            | mean     | sd   | min  | median | max  |
| ------------------------------------------------- | -------- | ---- | ---- | ------ | ---- |
| Initial consonant                                 | 68.3     | 13.8 | 25.9 | 72.7   | 89.4 |
| Vowel                                             | 96.0     | 4.5  | 80.6 | 96.3   | 100  |
| Final consonant                                   | 59.6     | 23.2 | 10.3 | 64.9   | 95.2 |
| **Consonants only** (init+final)/2 — _PCC analog_ | **63.9** | 17.6 | 29.0 | 70.5   | 92.3 |

Applying Burgoyne's TD-derived thresholds to the DS consonants-only analog: **96 %** of the DS sample falls at or below the PCC ≤ 85 % cut that identified only 6.88 % of the TD cohort; **71 %** fall at or below the TD difficulty group's _mean_ (78.3 %); and **29 %** fall at or below **53.5 %** — i.e. below the single worst-affected child in the entire 569-child TD sample. The DS position on the underlying pattern is also the expected one: final consonants worst (mean 60, min 10), initials next (68), vowels intact (96) — the final-consonant-deletion / cluster-reduction signature. **What is a 6.88 % clinical tail in TD is the near-universal baseline in DS**, despite the DS children being ~2 years older.

This buttresses the SP-flip two ways. **(i) Speech cannot enter the DS graph as a rare binary dummy.** Burgoyne could treat speech as a 0/1 minority indicator because it _was_ a minority trait; dichotomising at the same threshold in DS puts 96 % of children in one cell and throws the signal away. In DS, speech is a **continuous, graded node** spanning the full range — which is how a reopened `SP` should enter. **(ii) The TD mediation was estimated off-support for DS.** Burgoyne's "speech → phoneme awareness → reading, fully mediated" was learned in children whose consonant production was ~78–100 % intact; our children sit 15–40 points lower, frequently below the TD sample's floor. Importing that mediated structure is exactly the out-of-support extrapolation this review warns against, and it compounds the inversion of break ②: the TD carrier (phoneme awareness / blending) is itself weak in DS, so it cannot carry a speech deficit that is now near-universal _and_ severe. The distributional gap is thus independent evidence for **① orient `SP` upstream** and against **② importing the TD mediator**.

Three caveats belong with the numbers if this is cited. (a) **Scoring alignment:** Burgoyne pools all 43 consonants into one PCC; we reconstruct the consonant PCC as (initial + final)/2 from position-scored data — the same construct, sub-scored differently. (b) **Norm reference:** these are raw percent-correct against a _TD-normed_ threshold, applied descriptively — not a diagnostic classification of the DS children. (c) **Direction of the age gap:** the ~2-year age advantage runs _against_ DS looking worse, so it strengthens rather than qualifies the contrast. (Data cut: `data/rli_data_long.csv`, t1 rows, 2026-07-09.)
...
- Gelman, A., & Stern, H. (2006). The difference between "significant" and "not significant" is not itself statistically significant. _The American Statistician, 60_(4), 328–331. https://doi.org/10.1198/000313006X152649
...
- Hayiou-Thomas, M. E., Carroll, J. M., Leavett, R., Hulme, C., & Snowling, M. J. (2017). When does speech sound disorder matter for literacy? The role of disordered speech errors, co-occurring language impairment and family risk of dyslexia. _Journal of Child Psychology and Psychiatry, 58_(2), 197–205. https://doi.org/10.1111/jcpp.12648
