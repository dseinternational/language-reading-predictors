...
critical eye on correlated predictors. Recorded notes were deliberately *not*
...
2. **"Predictors of progress" are, empirically, mostly "baseline level of the
   ...
   the *only* one above the noise floor — is the baseline value of the very
...
3. **Level models' high R² is concurrent-measurement tautology, not
   ...
   co-administered, same-construct test — twice a *subtest of the same
   instrument*: `aptinfo`↔`aptgram` (Action Picture Test; LRP18/LRP20) and
...
4. **Permutation importance is statistically fragile here.** In **every one of
   the 22 models**, *no* predictor has an out-of-fold permutation importance that
...
7. **Where feature selection was actually done (LRP01–10), it mostly did not
   ...
   *retained* noise/redundancy (LRP08, LRP09) and the structural points above.

**Bottom line:** the pipeline is sound and the recorded models reproduce, but the
gradient-boosting layer is best read as *exploratory and largely confirmatory of
regression-to-the-mean plus concurrent test correlations*. It does not, on this
...
- **Metrics convention used below.** Pooled out-of-fold **R²** (SS aggregated
  across folds vs each fold's *training* mean) is the honest generalisation
...
- **`stability_selection.csv` uses a different importance metric** (scikit-learn's
  ...
  `permutation_importance.csv` (OOF, neg-RMSE). They frequently *disagree* — e.g.
...
`aptinfo`(LRP19 .46), `deappfi`(LRP21 .82). In most, it is the *only* predictor
above the 0.005 noise floor. LRP01 is the lone exception — its recorded selection
*dropped* the baseline `ewrswr`, leaving `attend` as a (barely-positive) top
...
- **LRP02 `ewrswr` — R² 0.61 (strong).** Top `spphon` 0.123, `eowpvt`, `yarclet`.
  ...
  questionable *keep*). Pruning *helped* (full-32 R² 0.51). Dropped `b1exto`/`yarcsi`
...
- **LRP10 `celf` — R² 0.49 (strong).** Construct-driven: dropped top-2 vocab
  ...
  (full 0.47) → redundancy again. Retains `trog` at the *worst* importance
  (−0.052, rank 10/10) as a "language control" — actively unhelpful; reasonable to
  drop. 4 time-invariant retained.

### Outcome 6 — receptive grammar (TROG) — *no FS done*
...
### Outcome 7 — nonword reading (NONWORD) — *no FS done*
...
### Outcome 8 — phoneme blending (BLENDING) — *no FS done*
...
### Outcome 9 — expressive grammar (APTGRAM) — *no FS done*
...
### Outcome 10 — expressive information (APTINFO) — *no FS done*
...
### Outcome 11 — fine articulation (DEAPPFI) — *no FS done*
...
1. **Reframe the gain models honestly.** Report them as showing
   ...
   *baseline-adjusted* gains or a level-at-t+1 controlling for level-at-t
...
6. **Trust, with caveats, the recorded selection decisions for LRP01–10.** They
   ...
   sound. The residual issues are *retained* clutter (e.g. LRP02 `aptinfo`, LRP09's
