...
| Outcome                              | Best est. (items) | 95% credible range | P(effect > 0) | Evidence     | Big enough to matter (Δ) |
| ------------------------------------ | ----------------- | ------------------ | ------------- | ------------ | ------------------------ |
| **L** — letter sounds                | **+3.53**         | [+1.22, +5.78]     | 0.999         | very strong  | 0.904 (Δ=2) — clears     |
| **W** — word reading                 | **+2.37**         | [+0.30, +4.48]     | 0.987         | strong       | 0.903 (Δ=1) — clears     |
| **P** — (heavily floored)            | +1.82             | [−1.81, +5.51]     | 0.838         | suggestive   | no bar defined           |
| **TE** — taught expressive vocab     | +1.57             | [+0.17, +2.95]     | 0.986         | strong       | 0.789 (Δ=1) — clears     |
| **TR** — taught receptive vocab      | +1.32             | [−0.15, +2.76]     | 0.961         | moderate     | 0.667 (Δ=1) — clears     |
| **R** — (receptive language)         | +1.01             | [−4.04, +6.00]     | 0.655         | inconclusive | 0.349 (Δ=2) — no         |
| **B** — Braille-type                 | +0.91             | [−0.06, +1.84]     | 0.968         | moderate     | 0.426 (Δ=1) — no         |
| **UR** — not-taught receptive vocab  | +0.69             | [−0.17, +1.56]     | 0.942         | moderate     | 0.239 (Δ=1) — no         |
| **UE** — not-taught expressive vocab | +0.38             | [−0.51, +1.26]     | 0.799         | suggestive   | 0.087 (Δ=1) — no         |
| **E** — (expressive language)        | −0.08             | [−4.24, +4.13]     | 0.483         | inconclusive | 0.164 (Δ=2) — no         |

Reading this: best estimates run from a near-zero, very wide E (−0.08 items, interval spanning ±4) up to a tight, clearly-positive letter-sounds effect (+3.53, interval well clear of zero). The **interval widths matter as much as the medians** — R, E and P are wide and uncertain, whereas L, W, TE are narrow and pointed away from zero. Of the nine outcomes that have a "big enough to matter" bar (P has none), **four are more likely than not to clear it**: L (0.904), W (0.903), TE (0.789) and TR (0.667). The second-clearest effect after letter sounds is **word reading (W): +2.37 items, 95% [0.30, 4.48], 98.7% probability positive (strong)** — this was omitted from the first draft, which reported only letter sounds.
...
| Fit         | Contrast                            | Marginals (items, P>0, evidence)                       | Contrast estimate | 95% range     | P(contrast > 0) | Verdict          |
| ----------- | ----------------------------------- | ------------------------------------------------------ | ----------------- | ------------- | --------------- | ---------------- |
| **ITT-016** | TE − TR (modality)                  | TE +1.54, 0.986 (strong); TR +1.35, 0.966 (moderate)   | +0.8 pp           | [−7.5, +9.1]  | 0.572           | **inconclusive** |
| **ITT-015** | TE − UE (expressive generalisation) | TE +1.55, 0.985 (strong); UE +0.36, 0.787 (suggestive) | +3.4 pp           | [−6.0, +12.8] | 0.762           | **suggestive**   |
| **ITT-115** | TR − UR (receptive generalisation)  | TR +1.37, 0.968 (moderate); UR +0.73, 0.955 (moderate) | −0.3 pp           | [−9.7, +8.7]  | 0.471           | **inconclusive** |

On the logit scale the same three contrasts are +0.069 [−0.32, +0.47] (016), +0.186 [−0.26, +0.63] (015) and −0.048 [−0.50, +0.39] (115). Reading them:
