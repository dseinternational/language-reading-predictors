...
The prepared 97-participant Byrne reading-language-memory extract is confirmed as a faithful derivative of the authoritative 97-case SPSS source. The separate 96-row CSV is an incomplete later derivative that omits one Down-syndrome participant; it is not evidence that an unexplained participant was added to the prepared extract. The dataset-level source-provenance gate may therefore be marked confirmed. This decision does not resolve provisional instrument ceilings, the `basmat` identity question or the missing visual-recall measures.
