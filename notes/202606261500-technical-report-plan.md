...
evidence table are designed to _seed_.

This note fixes the _structure_ only. It does not write chapters or scaffold files.
...
The report is largely an _operationalisation_ of `METHODS.md`, cross-checked against
three external guidelines:

- **Gelman, Vehtari, Simpson, et al. (2020), _Bayesian Workflow_.** arXiv:2011.01808,
...
- **Kruschke (2021), Bayesian Analysis Reporting Guidelines (BARG).** _Nature Human
  Behaviour_ 5(10), 1282–1291, DOI `10.1038/s41562-021-01177-7`. The explicit reporting
...
- **Depaoli & van de Schoot (2017), WAMBS checklist.** _Psychological Methods_ 22(2),
...
Five checklist items that need _explicit_ placement (none is currently obvious and all
...
- **Preface** (`index.qmd`) — reader contract; the causal-status label legend; the
  preliminary-status banner; the AI-authorship Quarto callout. _Reuses:_ `CLAUDE.md`
  labelling rule.
...
- **Ch 1. The study and the questions (as estimands)** — RLI scope (language **and**
  ...
  questions stated as estimands. _Reuses:_ `intro.qmd` (currently TODO); `METHODS.md`.
- **Ch 2. The causal model: the locked DAG** — DAG figure + node legend; the empty-
  ...
  _Reuses:_ `notes/202606231600-dag-revision-consolidated.md` (**the DAG figure must be
...
- **Ch 3. A reader's guide to Bayesian results (the bridge)** — the key chapter for the
  ...
  decision ≠ NHST; no "significance"; evidence can _support_ a null); the `p ≈ 2(1−pd)`
  identity _with the warning that a pd threshold is a p-value in disguise_; direction vs
  magnitude + ROPE/δ + evidence ladder; Type-S/Type-M (winner's curse) at this sample
  size; **state the HDI-vs-equal-tailed choice here**. _Reuses:_ `METHODS.md` glossary;
...
- **Ch 4. Methods** — Beta-Binomial / logit likelihood and items-scale reporting; the
  ...
  validation status**. Code is described, not shown; it lives in the repo. _Reuses:_
  `priors.py`, prior plots, `floor.py`, `METHODS.md`.
- **Ch 5. Step 1 — gradient-boosting discovery, and its honest limits** — short. Role =
  predictor screen, _not_ inference; the regression-to-the-mean / baseline-driven
  ...
  _Reuses:_ `notes/202606201500-gb-replication-findings.md`; SHAP figures.
...
- **Ch 6. Computation and convergence** — sampler / software / versions / seeds; R-hat
  ...
  example plus a roll-up table across all reported models. _Reuses:_ `config.json`,
...
- **Ch 7. Predictive checks and fit** — prior-predictive (priors cover without
  ...
  the binary estimand for floored outcomes. _Reuses:_ prior/posterior predictive plots,
...
- **Ch 8. The headline ITT effects** — one section per outcome (word reading;
  ...
  failure and not "the model ignores vocabulary." _Reuses:_ `tau_summary.csv`,
  `rope_summary.csv`, off-floor mover counts, the measurement-sensitivity audit.
  _(Map outcome → LRPITT id against the registry at drafting time.)_
- **Ch 9. Consistency across outcomes (joint view)** `[CAUSAL]` — joint-model forest;
  ...
  untaught generalisation contrast (limited transfer). _Reuses:_ `tau_forest.png`,
...
- **Ch 10. Robustness and within-person replication** — waitlist-crossover DiD
  ...
  where not yet run). _Reuses:_ `did_summary.csv`, robustness `tau_summary.csv`.
...
- **Ch 11. Alternative estimation views** — gain-factors (the treatment term reproduces
  ...
  per-protocol (no causal term; age-at-onset confound). _Reuses:_ `factor_summary.csv`,
  `cohort_marginal.csv`.
- **Ch 12. Mechanism and moderation** — mechanism slopes ("associated with, after
  ...
  _Reuses:_ `mechanism_curve.csv`, interaction summaries, LOO-compare CSVs.
- **Ch 13. Mediation (triangulation only)** — g-formula NDE/NIE, **not** point-
  ...
  _Reuses:_ `mediation_summary.csv` and its sensitivity variant.
...
- **Ch 14. Model comparison and build history** — PSIS-LOO with `elpd_diff` vs SE and
  ...
  Random Forest, floor-rule provenance) for multiplicity transparency _in the main text_,
  not buried. _Reuses:_ LOO CSVs; model-selection notes.
- **Ch 15. What it all means** — the causal core restated; the associational ring
  re-filed as _hypotheses to test_, not findings; the practical-implications thread that
  the secondary publication will expand. _Reuses:_ findings notes.
- **Ch 16. Limitations and future work** — sample size / winner's curse; latent-ability
  ...
  deferred DAG decisions; reporting-plumbing debts. _Reuses:_ the DAG note's open-
...
- **App B. Full model catalogue** — roll-up of all eight families / ~90 models with
  ...
  companions. _Re-read the CSVs into the report; do not transclude the per-model qmd
  (front-matter / relative-path conflict)._
...
- **Phase 0 — unblock the build.** Make **PDF the canonical format** in `_quarto.yml`
  ...
  _vocabulary_ to _language and reading_. Populate `references.bib` with the verified
...
- **Phase 3 — hand-write the narrative.** The plain leads, the vocabulary-null section,
  and Ch 15 synthesis — paraphrasing the _same_ label strings the number panels use, so
...
1. **Document architecture →** the technical report stays a _technical_ report (primary
...
- **Title / scope** — book `title` widened to _language and reading_, `subtitle` added,
