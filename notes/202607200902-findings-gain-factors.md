...
The family answers two questions the available-case modified ITT suite does not tackle in one model. **First**, it gives a second, differently-built estimate of the intervention effect: the **on-intervention treatment marginal** — the average change in proportion-correct caused by being on the intervention, _averaged over the period-1 transition only_ (the genuinely randomised, all-untreated-baseline transition targeted by the available-case modified ITT estimate). This is the single quantity in each model that randomisation licenses us to call **causal**, subject to its analysis-set assumptions. **Second**, and just as important, it reports a whole panel of **adjusted associations** — how each child's own baseline, age, cognitive ability, and the baseline levels of _other_ skills relate to how much they gain — which describe _who progresses_, not what the intervention does.
...
## Comparison with the available-case modified ITT suite — does the ANCOVA reproduce the fitted result?

The gain-factor treatment marginal and the available-case modified ITT estimate τ are built differently (a period-stacked ANCOVA versus a single-outcome available-case modified ITT model) but target the same randomised, all-untreated-baseline contrast among their fitted cases, so they should agree. Both are reported here on the proportion-correct scale (the available-case modified ITT figures come from each outcome's `tau_summary.csv`; F and T have **no** model in the core ITT-001–011 suite, so there is no direct comparator).

| Outcome                | Gain-factor marginal, proportion [89% CI] (evidence) | Available-case modified ITT τ [89% CI] | Verdict                           |
| ---------------------- | ---------------------------------------------------- | -------------------------------------- | --------------------------------- |
...
The reanalysis **broadly reproduces the available-case modified ITT result**. The two headline benefits — **word reading** (+2.6 words versus the available-case modified ITT estimate of +2.4 words) and **letter-sound knowledge** (+3.3 versus +3.5 sounds) — come out almost identically, and the two flat vocabulary outcomes (R, E) are flat and negligible in both. The systematic difference is that the gain-factor marginals are **slightly attenuated** on the taught-vocabulary and blending outcomes (TR moves from moderate in the available-case modified ITT fit to inconclusive here; TE and B drop one rung), consistent with the ANCOVA's heavier conditioning (own baseline + upstream skills + ability) absorbing some of the signal, and with the period-1-only averaging using fewer transitions than the single-window available-case modified ITT contrast. The direction never reverses on any outcome where either model has more than inconclusive evidence.

## What the family concludes

The gain-factor ANCOVA delivers a **second, differently built causal estimate under its stated assumptions** that agrees with the clearest available-case modified ITT estimates: strong-to-very-strong benefit on **word reading** and **letter-sound knowledge**, weaker-but-positive signals on **blending, basic concepts and taught expressive vocabulary**, and **no detectable effect on broad standardised vocabulary (R, E)**. This is the same gradient the available-case modified ITT suite and the difference-in-differences family report, so the headline is robust to how the model is built. The heavily-floored spelling (PS) and nonword (NW) outcomes remain inconclusive in every framing.
