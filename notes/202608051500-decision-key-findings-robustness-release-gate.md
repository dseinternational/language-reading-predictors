...
**Evidence that lifts a withhold.** For floor-rule outcomes, the provenance-validated six-cell grid the existing gate already recomputes. For ordinary ITT, a `tau_prior_sensitivity.csv` sweep **co-located with the fit**. Co-location is the point: the trace-validated 44-cell standard sweep is strong evidence, but it is a cross-model artefact living outside any one fit's output directory, which is exactly the separation the review flagged. Requiring the file beside the fit is what makes the evidence _that fit's_. A lifted withhold becomes `qualify` — the finding ships, labelled prior-informed and exploratory.
