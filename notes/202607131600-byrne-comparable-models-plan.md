...
4. **Measurement gates are now measure-specific.** Confirmed Beta-Binomial denominators are `basread=90`, `bpvs=32`, `trog=20`, `basdig=34`, `bassim=21` and `basmat=28`; `basspel=18`, `woco=31` and `basnum=60` remain observed-maximum placeholders with `n_trials_confirmed=False`. The checksum-pinned source's native `BASMAT3`-`BASMAT5` fields resolve the previous `basmat` identity caveat; the Raven comparison came from a different cohort. The Phase C reverse targets (`basread`, `bpvs`, `trog`, `basdig`) are all in the confirmed set, so the three manuals do not block that narrow question. The earlier 96-versus-97 extract-lineage discrepancy was reconciled on 2026-08-16 against the checksum-pinned 97-case SPSS source (`notes/202608161340-byrne-source-provenance-reconciliation.md`); fresh fits inherit that confirmation, while stored fits retain their original input snapshot. The same audit establishes that immediate and delayed visual recall are absent from the retained source, so the paper's full memory battery remains unreproducible from this archive (`notes/202608161554-byrne-basmat-and-visual-recall-audit.md`).
