...
Per outcome: children with a non-missing score at each wave, and **single-leg** attrition across the transitions the headline estimands use — the randomised **ITT window** (t1 baseline but no t2 post), the **t2→t3 crossover leg** (t2 but no t3), and the **maintenance leg** (t3 but no t4). The columns are per-leg, so the full DiD crossover chain t1→t2→t3 is complete for an outcome iff both its ITT-window and t2→t3 columns are 0. Cohort n = 54.
...
- **The DiD crossover chain (t1 → t2 → t3) is complete** for every graded outcome (both the ITT-window and t2→t3 legs are 0). The single t2→t3 gap is one child on nonword reading (N), a floored, post-only measure.
...
What little missingness exists is **intermittent, not monotone dropout**: the one phonetic-spelling gap is a child observed at t1 _and_ t4 but not t2/t3 (they return), and nonword's t1 gap is the post-only convention. There is no systematic drop-out pattern for a missingness model to exploit — which, if anything, strengthens the read below.
...
The table above is a hand-recorded snapshot and the CSV is gitignored, so on any change to `data/` or `MEASURES` regenerate it and re-check this note against the printed `attrition_audit.csv`.
