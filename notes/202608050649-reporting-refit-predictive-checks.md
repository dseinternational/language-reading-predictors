...
> [!IMPORTANT]
> **Corrected 2026-08-05** (see follow-up 10). The counts below are the re-measured ones, and the reading of the `corr_factor` concentration in the original version of this section was wrong — it is the per-child latent scores, not the loadings. The superseded text is quoted in the correction paragraph so the change is visible rather than silent.

Across 2,555 parameters: **1,100 clear**, 981 "potential prior-data conflict", 474 "potential strong prior / weak likelihood". Note that `psense_summary.csv` writes a tick `✓` for an _unflagged_ parameter, so the tick is the clear verdict, not a warning.

The prior-dominated parameters concentrate sharply: **368 of the 474 sit in just three `corr_factor` models** (`mm-001/002/101`, 118–127 of ~176 parameters each). This section originally read that as "a weakly identified latent-factor model in which the data do not pin the loadings, so the prior carries them". **That is backwards.** Broken down by parameter, every one of `mm-001`'s 123 prior-dominant rows is a `factor_z` — a _per-child latent factor score_. All six loadings (`lambda_load`), all six residual scales (`sigma_indicator`), all three factor correlations and every structural coefficient (`beta_factor`, `alpha`, `gamma_own`, `kappa`, `beta_age`, `beta_blocks`) come back clear. The deliverables are the pinned part; the shrunken part is the 153 child-level scores, each informed by two or three indicators, which a hierarchical prior is _supposed_ to pull toward the factor mean at n ≈ 51. That is the same construction-not-defect exemption this note already grants the `horseshoe` family (max prior sensitivity 0.99, because a shrinkage prior is designed to move under power-scaling).

The original text also said "the same three failing R-hat and ESS". They no longer fail: after the `LKJCorr` remediation all three pass cleanly (0 divergences, max R-hat 1.0017, min ESS 4,705). The coincidence that motivated reading the two together has gone.

Worth noting alongside: `rlm-mm-001` — the one model #381 exempted from power-scaling, on the grounds that its posterior had not converged — turns out to be the **cleanest** of the four measurement fits, with 4 flagged rows of 22 and **no** prior-dominant parameter at all.
...
9. ~~Apply the robustness release gate to this sweep's stored fits~~ **done 2026-08-05** — the gate (#475) merged at 14:09, after this sweep finished at 06:49, so every `key_findings.json` here predated it and all 28 ITT fits still carried an unqualified status `ok`. Regenerating with `scripts/regenerate_key_findings.py all` applies it without refitting: **2 withheld** (`itt-009` PS and `itt-011` NW, whose floor-rule treatment-prior grid is absent), **11 release with the prior-attenuation sentence**, 15 unchanged apart from the release audit block, and the other 166 fits byte-identical — the gate is ITT-scoped and touched nothing else.

   Two things this exposed that the regeneration alone would not have fixed. Each fit directory carries its **own copy** of `docs/models/_partials/`, taken at fit time, so all 194 held the pre-#475 `_key_findings.qmd`: on first re-render the withheld fits fell through to the soft "a key-findings summary is not available for this fit … see the results section below" note and, because that branch does not set `_scientific_results_released = False`, **published the very tables the gate withholds**. Syncing the partial and re-rendering suppresses them properly (`itt-009` goes from 15 tables and 24 figures to 5 and 15, with the diagnostic material retained by design). The copied-partial staleness is general: any report-layer change lands on stored fits only if the partials are re-synced, not merely by regenerating the JSON.

   The second is a wording defect in the gate itself. The floor withhold reason ended "… cannot support a cause-and-effect statement" and the callout appends ", so no cause-and-effect statement is released", so the published report said it twice. The reason now stops at the state of the evidence and names the file up front.

10. ~~Close the psense coverage gap (#381)~~ **done 2026-08-05** — psense now covers **186 of 194 fits and 21 of 22 families**. The eight remaining gaps are the treated-only `gain_factors` companions (`gf-101`–`gf-108`), which drop `beta_trt` because the treatment indicator is constant, so the family's single power-scaled term does not exist in them — a structural exemption, not a gap to chase.

    Two of the three missing families needed only wiring: `fit_joint` never called `run_psense` at all (#381 lists it under "ready — just add the call"), and `fit_rlm_corr_factor` called neither it nor `compute_log_likelihood_and_prior`. Both added; the four `joint` fits now report `tau` per outcome and flag L, B and P, which agrees outcome-for-outcome with the single-outcome ITT fits.

    **The third was a silent regression, and it is the part worth remembering.** `mm-001/002/101` do call `compute_log_likelihood_and_prior(strict=False)`, and they emitted psense before the remediation. Switching them from `LKJCholeskyCov` to bare `LKJCorr` (follow-up 4) tripped the #453 value-variable naming seam — `cholesky_corr` contains an underscore, so `get_untransformed_name` recovers the wrong name — and `log_prior` began failing. Because the psense caller is deliberately non-strict, that degraded to a yellow warning inside a 194-model sweep and the artefacts simply stopped appearing. **A fix that clears a convergence gate can silently cost a diagnostic, and a `strict=False` guard is exactly where that hides.** #475's `log_density_model` repair fixes the seam; all five affected fits were re-emitted with `--reuse-trace` (no re-sampling, every gate verdict unchanged).

    Still open on #381: `prior_pushforward.csv` remains at 63 of 194 and only four families. Every family the issue names for it — `dose`, `al`, `surv`, `ca`, `adj`, `rlm-hg`, 38 fits — is still at zero, as is the indicator-scale prior-predictive check for the CFA families.
