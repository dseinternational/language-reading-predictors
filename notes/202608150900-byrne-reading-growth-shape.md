...
The registered model now requires a trace-backed observation-cell influence sensitivity whenever any Pareto-k value exceeds the recorded threshold. It maps each flattened likelihood contribution back to its child, wave and outcome, masks all flagged cells simultaneously, refits the same model at the parent sampling tier, and compares the separate marginal posteriors for `gamma` and `delta`. This is a coefficient-stability check, not exact LOO and not leave-one-child-out prediction: a child remains in the model through any unflagged waves and is removed only when every observed cell for that child is flagged. Release remains `inputs_unresolved` because the prepared 97-participant extract has not been reconciled with the separate 96-participant raw export.

## Completed influence check

The `rep-lite` primary again passed with zero divergences, maximum R-hat 1.0044 and minimum effective sample size 1,953. Eight of 249 observed child–wave cells exceeded Pareto-k 0.70, spanning seven children; one child was removed from the sensitivity panel because both of their available reading waves were flagged. The refit passed with zero divergences, maximum R-hat 1.0021, minimum effective sample size 1,849 and minimum BFMI 0.636. The `gamma` median moved by +0.012 logit units and retained its sign, with overlapping 89% intervals; `delta` moved by +0.135 and likewise retained its sign with overlapping intervals. This resolves the requested coefficient-stability check but does not make the original PSIS-LOO score reliable for model comparison. The source-provenance gate continues to suppress the coefficient table and all scientific interpretation.
