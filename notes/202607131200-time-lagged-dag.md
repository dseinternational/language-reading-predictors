...
- **Reverse / reciprocal edges added** (all lagged; target set directed by the team 2026-07-13): `WR→TE`, `WR→TR` (reading feeding back into the **taught** vocabulary measures; broad vocabulary is then reached indirectly via the within-wave `TE→EV` / `TR→RV` paths, so the founding RLI reading→language hypothesis — the critical review's "most consequential missing structure" — is routed through the taught words), `WR→PA` (reciprocal with `PA→WR` — enables the cross-lagged dominance test), and `WR→RW` (reading → phonological memory). Each edge still needs its one-line justification for the record. An earlier draft pointed the vocabulary feedback at the standardised measures (`WR→EV`/`WR→RV`); superseded by this set.
