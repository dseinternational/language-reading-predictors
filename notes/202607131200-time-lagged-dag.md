...
> [!NOTE]
> Drafted by an LLM-based AI tool (Claude Code/Opus 4.8); revised for the adopted Option A structure by Claude Code/Fable 5 and for the 2026-08-08 `WR → NW` decision by Codex/GPT-5.
...
2. **Reading → language coexists with language → reading** without a cycle — `WR_t → TE_{t+1} → EV_{t+1}` (the founding RLI hypothesis, routed through the taught words) alongside the within-wave `EV → PA → WR` cascade.
...
- **Reverse / reciprocal edges added** (all lagged): `WR→TE`, `WR→TR` (reading feeding back into the **taught** vocabulary measures; broad vocabulary is then reached indirectly via the within-wave `TE→EV` / `TR→RV` paths, so the founding RLI reading→language hypothesis — the critical review's "most consequential missing structure" — is routed through the taught words), `WR→PA` (reciprocal with `PA→WR` — enables the cross-lagged dominance test), and `WR→RW` (reading → phonological memory) were directed by the team on 2026-07-13. `WR→LS` was added on 2026-07-17 for the reverse-direction test ([decision record](202607172100-reverse-mediation-wr-ls-direction-spec.md)). `WR→NW` was added on 2026-08-08 as a provisional working-DAG assumption: word knowledge may support later decoding through visual analogy or another route not represented by letter sounds and blending ([decision record](202608081900-decision-wr-nw-lagged-edge.md)). That final edge does not make the association causal or fittable here: the minimal measured backdoor sets remain 8 and 12 nodes. Each original edge still needs its one-line justification for the record. An earlier draft pointed the vocabulary feedback at the standardised measures (`WR→EV`/`WR→RV`); superseded by this set.
...
Verified acyclic (36 nodes, 197 edges).
...
- **Cross-lagged dominance tests** — the forward `PA→WR` path vs the lagged `WR_t→PA_{t+1}`; reading→language via the taught words (`WR→TE`/`WR→TR`) — become first-class hypotheses the LCSM machinery can probe.
