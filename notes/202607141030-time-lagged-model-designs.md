...
Concretely: each measure's change equation gets **arm × window change intercepts** `a_m[g, w]` (2 arms × 3 transitions = 6 cells per measure, each cell informed by 26–28 children), replacing the single pooled `a_change`. A free by-product: the window-1 cell contrast `a_TE[imm, w1] − a_TE[wait, w1]` is an available-case modified ITT contrast on latent taught-vocabulary change, directly comparable in direction and rough magnitude with `lrp-rli-itt-002`'s available-case modified ITT estimate τ — a built-in consistency check between the new family and the ITT suite.
...
- **Estimand labels:** `g_WTE`, `g_WTR` are **adjusted associations** (latent `GA` is unblockable; the LCSM's independent initial statuses absorb only part of the stable between-child differences, and the RI-CLPM that would separate them is not estimable at n ≈ 54 — same honesty box as `lcsm-067`/#273). The window-1 arm contrasts `a_m[imm, 1] − a_m[wait, 1]` are available-case modified ITT contrasts on latent change, reported as consistency checks against `lrp-rli-itt-001/002`, not as new headline effects.
