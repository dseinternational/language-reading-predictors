...
**`WR → NW` is not a direct edge at all.** The lagged DAG has `WR_t → {TE, TR, PA, RW, LS}_{t+1}` and no `NW`. Any `WR → NW` coupling estimated on this graph is the _total_ lagged effect routed through letter sounds and blending — and conditioning on those, as some candidate sets do, both blocks the paths being sought and opens colliders. So the strongest descriptive signal in this note corresponds to **no edge in the DAG**. Either the graph is missing `WR_t → NW_{t+1}` — which Roch & Jarrold and the floor-exit result above would both support — or the effect genuinely runs through letter sounds and blending, in which case it should be visible there and mostly is not.
...
**The one dedicated blending → reading mechanism model failed its gate.** `lrp-rli-mech-190` — the only model whose focal exposure is blending with word reading as the outcome, and so the only one fitting that relationship as a flexible curve rather than a single adjusted coefficient — has **31 divergences** (R-hat 1.001 and min ESS 2 621 are both fine, so it is a geometry problem, not a length problem). Its curve is therefore unusable, and there is no estimate of the _shape_ of the blending–reading relationship: whether it has a knee, as letter sounds do near the top of their range — the letter-sound knee sits at about 29.5 of 32, though with a wide credible range and partly manufactured by the bounded scale on a logit link ([skill thresholds](202607171215-findings-skill-thresholds.md)). That is the model one would want for a threshold question, and it needs a reparameterisation before it can answer one.
...
**First, the nonword DAG question — a decision, not a model.** Whether `WR_t → NW_{t+1}` belongs in the lagged graph is the one open item here that blocks everything downstream, and it is a team decision rather than a build. The evidence for it is the Roch & Jarrold DS-specific finding plus the 62 %-vs-10 % floor-exit result above; the evidence against is that the current graph routes reading's influence on nonword reading through letter sounds and blending, where it should then be visible and mostly is not. Until it is settled there is no identified estimand to fit, and the ≥ 8-node adjustment set the current graph implies would be unfittable at this sample size anyway. This is the highest-value next step because it is cheap and it determines whether the strongest descriptive signal in the note has any home at all.

**Second, `WR → LS` — optional, and weaker than it first looks.** A reverse-coupling LCSM with a no-coupling LOO comparator (processes `W` and `L`, coupling `g_W_L`, arm × window change intercepts, and the four-to-five node adjustment set derived above) is genuinely cheap — two processes rather than `lcsm-081`'s three, and a smaller covariate block. What it would add over `med-176` is pooling across all three transitions instead of one t2 → t4 window, and a per-transition coupling rather than an intervention-effect share. What it would **not** fix is the reason to hesitate: `med-176` already answers the direction question with moderate evidence, and its fragility to unmeasured confounding (tipping at 30 % of the mediator → outcome path) is a latent-general-ability problem that a second model with the same unblockable confounder inherits wholesale. A cheap adjustment set buys freedom from _measured_ confounding; it buys nothing against `GA`. Build it if the per-transition estimand is independently wanted, not in the expectation that it will firm up `med-176`. Model ids `lcsm-083`/`lcsm-183` are **not** free (83 and 84 are dose-response); a free pair needs picking against `definitions.MODEL_REGISTRY`.
...
**A repair, not a new model: `mech-190`.** The one direct blending → word-reading fit in the suite fails its gate on 31 divergences, so pass 4 has no estimate of the relationship's _shape_. Since R-hat and ESS are both healthy, this is a geometry problem — the usual first moves are a non-centred GP parameterisation, a tighter lengthscale prior, or raising `target_accept` — and it is worth doing, because the knee question is exactly where blending and letter sounds might differ most and it is currently unanswerable. One trap for whoever picks it up: **the module's docstring header calls itself "LRP91", but `lrp91` is the registry alias of `lcsm-091`** — the change-on-change model quoted earlier in this note, an unrelated fit. The correct alias is `lrp190`. Its neighbour `lrp_rli_mech_191.py` has the same defect (header "LRP92", while `lrp92` is a registered mediation model). Trust `definitions.MODEL_REGISTRY`, not the docstring headers, in the `mech-19N` block.

## A maintenance finding

A small one, and narrower than it first appeared. [`202607141030-lagged-dsep-checks.py`](assets/202607141030-lagged-dsep-checks.py) — the asset the reverse-coupling design note points readers at — has a `REVERSE` list of `["TE", "TR", "PA", "RW"]`, missing the `WR_t → LS_t1` edge added on 2026-07-17. Re-run today it therefore builds a graph the DAG no longer matches.

**The CI guard is fine**, which is the part worth being clear about. The design note's recommendation to promote the checks to a pytest **was** actioned: `tests/test_lagged_dag_adjustment_sets.py` carries the corrected five-edge `REVERSE` list including `LS`, and `test_unroll_slices_mirror_the_dagitty_template` asserts that every edge in the parsed `.dagitty` template appears in the unroll — exactly the drift described above. It passes. So the invariant is guarded; what went stale is only the **archived reproducible record** attached to the note, which was not updated alongside the test when the `LS` edge landed.

Nothing published is wrong either way: the new asset re-runs the design note's key derivations with letter sounds restored and they all still hold (`WR → TE` invalid without arm, valid with; `WR → TR` valid). The remaining fix is small — refresh the 2026-07-14 asset's edge list, or replace it with a pointer to the test that supersedes it.
...
- [Skill thresholds](202607171215-findings-skill-thresholds.md) — the letter-sound knee that `mech-190` would have to be repaired to test for blending.
- Fitted models quoted: `lrp-rli-ca-001`, `lrp-rli-gf-001`, `lrp-rli-lcsm-081`, `lrp-rli-lcsm-082`, `lrp-rli-lcsm-091`, `lrp-rli-med-066`, `lrp-rli-med-075`, `lrp-rli-med-076`, `lrp-rli-med-176`, and `lrp-rli-mech-190` (gate-failed, quoted only as a gap).
