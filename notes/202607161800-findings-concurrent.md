...
The weakest-associated focal skill; only two adjusted coefficients exclude zero. **Taught expressive vocab at t3 is the largest, +1.3 items (+0.4 to +2.2), 0.9898 → strong**, and **Receptive vocab at t4 is the clearest, +1.1 (+0.4 to +1.8), 0.9908 → very strong** (the headline). Receptive vocab is otherwise suggestive/moderate — t1 +1.2 (0.969 → moderate), t3 +0.9 (0.968 → moderate). **Letter sounds at t1 is negative, −0.6 (−1.4 to +0.2), P(>0)=0.120** — directionally negative but interval crosses zero, so inconclusive. Bivariate TR associations are uniformly larger (≈ +1.2 to +2.5 items, all ≈ 1.0000).
