...
**Moderation.** Whether the letter-sound association varies with age (`mech-073`) or phonological memory (`mech-104`, `204`) was tested, and the two answers differ. The age interaction is −0.057 with P(negative) = 0.93 — **moderate evidence that the adjusted association is weaker for older children**, though its 89% interval still includes zero; this is associational moderation, not causal effect modification. The phonological-memory interaction is −0.008 with P(negative) = 0.55, which is **inconclusive**; a formal predictive comparison of the memory-moderation pair likewise returned "inconclusive (|elpd_diff| < 4)". Neither result licenses a claim that the intervention or a causal decoding route works differently for different children. Two cautions apply to every interaction term in this family (2026-08-19). First, `gamma_int` is a product term on the latent logit scale, and on a bounded outcome its sign is not a statement about items: these children read in the bottom fifth of the word-reading scale, where the logit is concave, so two positive effects that are additive in words show a negative logit product. Each moderated fit therefore also writes `moderation_items.csv`, the surface re-expressed in items at the interquartile cells of the fitted rows; for age the letter-sound increment from 17 to 28 sounds is worth +3.6 words at 77 months and +2.7 at 101, a difference of −1.0 words [−1.8, −0.1] (P(negative) = 0.96), so the weaker association for older children holds in words too, while the phonological-memory contrast is −0.2 [−1.8, +1.4], inconclusive on both scales. The joint-readiness interactions (`mech-061`, `063`, `071`, `093`–`095`) are read in the question-organised report (question 5): in words only the blending interaction survives (−1.1 words [−2.3, −0.0], P = 0.95); decoding and the four vocabulary moderators are inconclusive. Second, every moderated letter-sound → word-reading fit returns a negative interaction of about the same size whatever the moderator — blending −0.11, taught expressive −0.09, expressive −0.07, taught receptive −0.06, age −0.06, decoding and receptive −0.05 — and the Haldane logit stretches the letter-sound ceiling (the top four of 32 sounds span 1.6 SD of the standardised exposure, the middle 23 span 2.4), so the 18% of rows at 30 or more sounds carry between about half and four-fifths of the product term's leverage. Some of that uniform negativity may be curvature at the letter-sound ceiling that the six-function curve cannot express rather than moderation; the check, if wanted, is a one-dimensional one (a moderated refit with the interaction on the count-standardised or top-clipped letter-sound score), not a two-dimensional surface.
