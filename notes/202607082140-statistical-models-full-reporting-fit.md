...
- **Causal vs association.** Randomisation identifies the full-cohort assigned-arm contrast in principle. The fitted τ is an **available-case modified ITT estimate**, with a causal reading only under the stated selection assumption; the difference-in-differences δ and gain-factor on-intervention term have their own stated identification conditions. Every other coefficient — baselines, ages, skill-to-skill couplings, mediator paths — is an _adjusted association_: informative about how skills travel together, but never to be read as "X drives Y".
...
## Headline causal findings — the available-case modified ITT estimates (`itt` suite)

The available-case modified ITT models ask: _among children represented in the archived analysis data, did being randomised to start the intervention immediately improve each outcome at the end of the randomised phase, compared with waiting?_ The trial randomised 57 children, but the archived t1 data contain 54 and outcome observation requirements reduce some fits further. Random assignment identifies the full-cohort contrast in principle; causal interpretation of the fitted available-case estimate additionally requires the stated selection assumption, and extension to all 57 requires a missingness assumption or sensitivity analysis. The graded models include each child's own baseline score and age purely to sharpen precision; the post-hoc P/N transition analyses use age only and baseline zero to define the observed subgroup. τ is reported on the risk-difference scale (median, 95 % credible interval, P(τ>0)); positive = the intervention helps.
...
The word-reading and letter-sound available-case modified ITT estimates survive both robustness adjustments and reappear, at consistent magnitude, in designs that do not share the same model assumptions. In plain terms: three different ways of slicing the same trial — comparing the fitted observed cases by randomised group (available-case modified ITT), comparing each child with themself before and during the intervention (DiD), and pooling every on- and off-intervention period into one adjusted gains model (gain factors) — all land on the same answer, subject to their respective assumptions.
