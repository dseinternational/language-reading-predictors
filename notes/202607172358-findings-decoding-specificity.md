...
>
> **The two rows are not fitted on the same sample or the same unit** (#591). `jm-002` requires both outcome baselines on every retained transition and standardises the letter-sound logit once over that joint union — 153 rows, exposure SD 1.412 — while `mech-096` filters to its own 152 nonword rows (SD 1.386) and `mech-101` to its own 156 word-reading rows (SD 1.434). One SD is therefore a slightly different raw increment in each fit, and the word-reading marginal keeps four rows the joint fit excludes. The closeness of +0.81 to +0.78 is reassuring, but it is **not** a measurement of what working independence cost: that gap mixes a dependence change with a sample and scale change. `scripts/compare_statistical_models.py` now publishes the row and scaler reconciliation beside both rows with an explicit `comparable` verdict.
>
...
Conditional on each outcome's own baseline, letter sounds predict **pure decoding about four times as strongly as they predict word reading**, and the identified joint contrast excludes zero decisively. That is _consistent_ with letter-sound knowledge being converted into decoding, and it is the pattern the decoding account predicts.

**But it is not a signature a common factor cannot fake** (#591). Write `LS`, `N` and `W` as loading on one latent general ability with mutually independent errors: the two latent-scale slopes remain proportional to their loadings, so Δ is proportional to the _loading difference_ even with no causal letter-sound route at all. The informal argument that `GA` "should predict the broader word-reading skill at least as much" is an assumption about loadings, not a property of the data — and these two outcomes are measured very differently (79 items against 6, with a severe nonword floor and different link discrimination), with no cross-instrument measurement invariance imposed anywhere in the model. Δ is therefore an **adjusted, measurement-scale-dependent association contrast**: it neither rejects a common-factor explanation nor identifies a mechanism. Ruling the common factor out would need an explicit structural alternative; putting the two outcomes on one scale would need an item-response or calibrated measurement model, which still would not settle the confounding question. **Decoding-use remains consistent with the data, not demonstrated.**
