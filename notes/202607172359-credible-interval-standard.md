...
2. **MCMC stability.** The limits of a 95 % interval are the **2.5th / 97.5th** percentiles — the noisiest quantiles to estimate from a finite set of posterior draws, because few draws land in the extreme tails. The 89 % limits (**5.5th / 94.5th**) sit further from the tails and are estimated far more stably at the effective sample sizes this suite attains. Kruschke's Bayesian Analysis Reporting Guidelines make the point directly: _"For reasonably stable estimates of limits of highest-density intervals (HDIs), I recommend that ESS ≥ 10,000. For stable estimates of equal-tailed intervals, ESS can be lower."_ (Kruschke 2021, [doi:10.1038/s41562-021-01177-7](https://doi.org/10.1038/s41562-021-01177-7)). Our per-parameter ESS is typically in the hundreds-to-thousands, well below the ~10,000 that would pin a 95 % limit to two significant figures, so the 95 % endpoints were the least trustworthy numbers we reported.
