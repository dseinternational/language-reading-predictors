...
**#552, `level_factors` centred on the timepoint-1 gap.** The re-parameterisation did what it was for. In August, power scaling flagged the treatment coefficient of two `level_factors` fits as prior-dominant and both needed a treatment-prior sweep before they could publish. Under the new parameterisation **all eleven fits publish and none needs one**, because the prior sensitivity has moved off the randomised contrast and onto the pre-randomisation balance quantity, where it says nothing about the effect. The focal `d_grp_time[t2]` is likelihood-dominated in nine of the eleven — for word reading (`lf-001`) prior 0.062 against likelihood 0.123, for letter sounds (`lf-004`) 0.078 against 0.145 — while `arm_gap_t1` carries the "strong prior / weak likelihood" flag in eight of the eleven (0.059 against 0.014 for word reading, 0.053 against 0.033 for letter sounds). The two fits whose focal term still responds more to the prior than to the likelihood are the floor-rule outcomes phonetic spelling and nonword reading, and the gate classifies neither as prior-dominant.
