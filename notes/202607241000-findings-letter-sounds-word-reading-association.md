...
2. **The share-retained column is a sensitivity, not an identified quantity.** The Q1 and Q2 slopes come from _separate_ fits, so the ratio is computed by pairing draws under a working-independence assumption — the same caveat as the Tier-1 Δ ([decoding-specificity note](202607172358-findings-decoding-specificity.md)). The two marginal slopes are each properly identified; only their ratio is not.
...
1. **A joint `{WR, NW}` levels model.** Turns Q2's share-retained from a product-of-marginals sensitivity into an identified posterior contrast — and fixes the Tier-1 Δ, which the [decoding-specificity note](202607172358-findings-decoding-specificity.md) already flags as non-identified for the same reason. Two notes now depend on this one piece of machinery, which makes it the best-value build in the list. Needs the `joint` factory extended to a levels design, or a two-outcome `concurrent` factory.
