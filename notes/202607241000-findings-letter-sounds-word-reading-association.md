...
> [!IMPORTANT]
> **Probe and production status (updated 2026-08-09).** All eleven models promoted from the exploratory work (`ca-010/011`, `gf-012/013`, `mech-102/103/104/204`, `jm-001/002`, `med-060`) have reporting-tier fits that pass the automatic convergence gate; the consolidated August findings notes are the current results synthesis. `ca-010/011` were freshly fitted at reporting tier in the pinned `df00982f53cc44f3635abaef01c4c09ecf185afe` run after the concurrent-family missingness repair; their registered results are separated below from the dated scratch probe. The July power-scaling flags on `jm-001/002` were **not covered by the later targeted prior-refit work**: they remain a warning that parts of the joint fits are weakly likelihood-identified or in prior–data conflict. Under the repository's current release policy, `joint_mechanism` is an observational family and those flags qualify interpretation rather than formally withholding release. This does not make the parameters prior-robust. In the scratch probe, `RW` and `SP` enter as same-wave states; mean-filled hearing, `RW` and `SP` covariates carry their required missingness nuisances; and fitted-row-constant flags are omitted rather than aliased with the intercept. The implemented registered models and their reporting fits completed the #421 work; the later #502 refit validates the corrected production concurrent-family contract rather than retroactively turning the scratch probe into a registered artefact.
...
**Status: exploratory scratch fits, not registered model artefacts.** They publish no `config.json` / `diagnostics_summary.json` / report and so bypass the production convergence-gate artefact, but the probe applies the same thresholds to unrounded diagnostics over every fit's free variables. All 62 fits pass: **maximum R-hat 1.0026, minimum bulk ESS 1,723, minimum tail ESS 2,861, minimum per-chain BFMI 0.739, and zero divergences** (thresholds R-hat ≤ 1.01, both ESS ≥ 400, BFMI ≥ 0.3, zero divergences). Displayed diagnostics are rounded only after the pass/fail decision. Where a registered counterpart now exists, cite its production report rather than treating this scratch fit as interchangeable; the scratch Q3 analyses remain a separate exploratory tier.
...
| ---- | ------------------------------- | ---------------------------------- | ---------------------------------- |
| t1   | +0.643 [+0.350, +0.941], 0.9997 | +0.615 [+0.328, +0.902], 0.9996    | +0.378 [+0.136, +0.602], 0.9916    |
| t2   | +1.070 [+0.819, +1.295], 1.0000 | +0.792 [+0.520, +1.056], 1.0000    | +0.358 [+0.173, +0.547], 0.9991    |
| t3   | +0.717 [+0.501, +0.929], 1.0000 | +0.578 [+0.362, +0.790], 1.0000    | +0.418 [+0.225, +0.605], 0.9993    |
| t4   | +0.813 [+0.626, +0.991], 1.0000 | +0.527 [+0.327, +0.726], 1.0000    | +0.515 [+0.312, +0.718], 0.9999    |

All 20 `ca-010/011` adjusted and single-skill fits pass the automatic gate: maximum R-hat 1.000385413581396, minimum effective sample size 18403.680002873145, minimum BFMI 0.9591989573432169 and zero divergences. Their primary-wave PSIS-LOO anchors have no Pareto-`k` exceedance. This establishes clean computation under the models, not causal identification or immunity to the nonword measure's floor.
...
## Q1 — `LS` and the `WR` level, holding age, hearing and non-verbal ability fixed (exploratory probe)
...
**Yes — the conditional association remains positive.** Holding the coarse observed nonword score fixed attenuates the `LS` slope by roughly a **fifth to a third** at t2–t4 (nothing at t1, where `NW` is almost entirely floored), but what remains is decisively positive at every wave: **P(residual `LS` slope > 0) = 1.000** in all four. This is not a mediated share: nonlinear conditioning, measurement error, latent general-ability confounding and possible collider bias prevent the retained slope from identifying a non-decoding channel.

Three qualifications, in order of how much they should change your reading:

1. **`NW` is a 6-item measure with a severe floor** — 80% of children score 0 or 1 at t1, and the median is still only 1 at t4. Conditioning on this coarse measure holds observed decoding fixed only partially. The residual `LS` slope is neither a formal upper bound nor a channel estimate, and better measurement could change it in either direction.
2. **The share-retained column is the original separate-fit sensitivity; the registered joint fit supplies a within-model readout, not identification.** The Q1 and Q2 slopes in this dated table come from _separate_ fits, so their ratio pairs draws under a working-independence assumption. [`lrp-rli-jm-001`](../docs/models/lrp-rli-jm-001/) instead fits word reading and nonword decoding **together at each wave** with an LKJ residual correlation, matched term-for-term to `ca-010` / `ca-011`, and reports `share_retained = β(LS→W | N) / β(LS→W)` as a within-model deterministic built from `β_W − ρ (σ_W/σ_N) β_N`. Its reporting fit passed the convergence gate; its report is the authoritative registered joint-model result, not proof that the estimand is causally identified. The two remain useful as a bracket rather than one correcting the other: `jm-001` conditions on the **latent** nonword logit where `ca-011` conditions on the observed count, and the ratio is meaningful only while the unconditional slope stays clear of zero. Power-scaling does not directly diagnose a ratio, so `share_retained` was excluded from that calculation; its component terms nevertheless carried the July weak-likelihood/prior-conflict flags. Treat the model-dependent share as qualified, not as proof that the prior is immaterial.
3. **`NW` is a mediator, not a confounder.** The revised DAG encodes `LS → NW → WR`, so adjustment changes the conditional question rather than controlling a pre-treatment common cause. The Q2 slope is the `LS`–`WR` association conditional on the observed `NW` count, not an identified direct path; because `NW` is a common effect of `LS` and other reading inputs, conditioning on it can also open a collider path. Read it as a sensitivity alongside Q1, never as a decomposition that corrects Q1.
...
This is consistent with the decoding-specificity result, which found the adjusted `LS`–`NW` association about four times steeper than `LS`–`WR` on a common logit scale. Together the fits show strong same-wave coupling among the code skills and a positive residual `LS`–`WR` association after conditioning on the coarse observed `NW` score; they do not identify the directions or channels linking those skills.
...
**Can say.** Letter-sound knowledge is by a wide margin the strongest same-wave correlate of word-reading level in this cohort; the association remains positive after adjustment for age, hearing, non-verbal ability and the coarse observed nonword score; the children who break the pattern — good letter sounds, poor word reading — are distinguished by phonological memory, decoding and, secondarily, oral language, not by non-verbal ability, hearing or youth; and vocabulary predicts learning new _spoken_ words strongly but learning to _read_ words only weakly — the same either way once letter sounds are held fixed.
...
| ------------------ | -------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lrp-rli-ca-010`   | `concurrent`   | `WR` level ~ `LS` + age + hearing + ability, per wave | Pinned 2026-08-09 reporting fit passes; adjusted `LS` slopes are +0.643 / +1.070 / +0.717 / +0.813 at t1–t4.                                                                                                               |
| `lrp-rli-ca-011`   | `concurrent`   | as `ca-010`, plus `NW`                                | Pinned reporting fit passes; adjusted `LS` slopes are +0.615 / +0.792 / +0.578 / +0.527 and `NW` slopes are +0.378 / +0.358 / +0.418 / +0.515 at t1–t4.                                                                    |
...
1. ~~**A joint `{WR, NW}` levels model.**~~ **Built** (#421 Tier 3 (1)), as two models rather than one, because the two quantities live on different parameterisations. [`lrp-rli-jm-001`](../docs/models/lrp-rli-jm-001/) is the per-wave levels model, matched to `ca-010` / `ca-011`, and reports Q2's share-retained as a within-model deterministic. [`lrp-rli-jm-002`](../docs/models/lrp-rli-jm-002/) is the phase-stacked ANCOVA companion, matched to `mech-096` / `mech-101`, and reports the Tier-1 Δ on the parameterisation the [decoding-specificity note](202607172358-findings-decoding-specificity.md) defines it on. Both reporting fits pass, but convergence is not identification. Their Δs differ because they answer different questions: the levels Δ is near zero or negative, whereas the conditional-change Δ is +0.81 [0.50, 1.14]. Both carry an LKJ cross-outcome dependence block; the remaining power-scaling flags are interpretive qualifications, not unresolved convergence failures.
