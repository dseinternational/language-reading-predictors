...
2. **The share-retained column is a sensitivity, not an identified quantity — but the identified version now exists.** The Q1 and Q2 slopes come from _separate_ fits, so the ratio quoted here is computed by pairing draws under a working-independence assumption — the same caveat as the Tier-1 Δ ([decoding-specificity note](202607172358-findings-decoding-specificity.md)). The two marginal slopes are each properly identified; only their ratio is not. [`lrp-rli-jm-001`](../docs/models/lrp-rli-jm-001/) (#421 Tier 3 (1)) closes this: it fits word reading and nonword decoding **together at each wave** with an LKJ residual correlation, matched term-for-term to `ca-010` / `ca-011`, and reports `share_retained = β(LS→W | N) / β(LS→W)` as a within-model deterministic built from `β_W − ρ (σ_W/σ_N) β_N`. Two things to carry over when the numbers are swapped in. First, `jm-001` conditions on the **latent** nonword logit where `ca-011` conditions on the _observed_ count; partialling the latent skill also partials its measurement error, so the identified share will generally be **lower** — read the two as bracketing rather than one correcting the other, and note this pushes in the same direction as qualification 1 above. Second, it is a ratio, informative only while the unconditional slope stays clear of zero. **This note still quotes the sensitivity** until `jm-001` has passed the convergence gate at `--config reporting`.
...
| Speech production (`SP`, t1 `deapp_c`) | +0.25 (0.94)   | +0.28 (0.99)   | +0.19 (0.94)     | +0.28 (0.99)     | modest, consistent                      |
| Phonological memory (`RW`, t1 `erbto`) | +0.29 (0.97)   | +0.27 (0.99)   | +0.15 (0.87)     | +0.20 (0.96)     | modest, consistent                      |
...
Fitting all of them **together** (heavily collinear at n ≈ 51, so read strictly under the Table-2 fallacy and not as a ranking) leaves only two standing at every wave: `LS` itself (+0.46 / +0.74 / +0.36 / +0.40, P ≥ 0.99) and `NW` (+0.24 / +0.26 / +0.34 / +0.40, P 0.94 → 1.000). Expressive vocabulary holds at t3–t4; the t1-baseline `SP` and `RW` terms collapse — note the asymmetry that they are t1 baselines competing against same-wave skills. Non-verbal ability goes flat or _negative_ in the joint fit (t3: −0.27, P(>0) = 0.043), the classic collinearity/suppression artefact the concurrent family's report warns about, and not a finding.
...
## Models worth formally building and fitting

Proposed ids are checked against the registry for collisions. **`lrp-rli-mech-090` (`RW → WR`) and `lrp-rli-mech-190` (`PA → WR`) already exist** — an earlier draft of this list proposed them, wrongly. `mech-090` puts phonological memory at **+0.10 logit per +1 SD (89% +0.00 to +0.20, P = 0.952)**, i.e. the Q3 discriminator is already registered on the `WR` side; what is missing is its **decoding** counterpart.

### Tier 1 — existing factories, no new pipeline code

| Proposed           | Family         | What it estimates                                     | Why this discussion asks for it                                                                                                                                                                                                                                                           |
| ------------------ | -------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lrp-rli-ca-010`   | `concurrent`   | `WR` level ~ `LS` + age + hearing + ability, per wave | Q1's headline currently lives only in a scratch probe: no gate, no report, no `key_findings.json`, no `psense`                                                                                                                                                                            |
| `lrp-rli-ca-011`   | `concurrent`   | as `ca-010`, plus `NW`                                | Q2's decomposition, same reason                                                                                                                                                                                                                                                           |
| `lrp-rli-gf-012`   | `gain_factors` | `TR` gains with `skill_symbols = ("R", "E")`          | `gf-009` carries **no** skill terms, yet Q4 makes broad receptive vocabulary the clearest predictor of taught-word learning anywhere in the probe (+0.38, P = 0.999)                                                                                                                      |
| `lrp-rli-gf-013`   | `gain_factors` | `TE` gains with `("TR", "R", "E")`                    | `gf-010` carries only `TR`; Q4 finds `RV` **and** `EV` both matter (+0.28 / +0.31)                                                                                                                                                                                                        |
| `lrp-rli-mech-102` | `mechanism`    | **`RW → NW`** (phonological memory → decoding)        | The sharpest Q3 discriminator was **nonword repetition** (_d_ ≈ −1.2). `mech-090` tests `RW → WR`; nothing tests whether phonological memory feeds the alphabetic route specifically. `mech-090`'s adjustment set ({A, GA, HS}, RW's only measured parent being `HS`) transfers unchanged |
| `lrp-rli-mech-103` | `mechanism`    | **`SP → NW`** (speech production → decoding)          | `SP` is a DAG parent of both `LS` and `NW` and a consistent Q3 discriminator (+0.19 to +0.28), yet there is **no** `SP`-exposure mechanism model in the suite                                                                                                                             |
...
| Proposed           | Family      | What it estimates                                                   | Value and caveat                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ----------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lrp-rli-mech-104` | `mechanism` | `LS → WR` **moderated by `RW`** (`gamma_int`)                       | Probably the highest-value model in this list: the direct, uncertainty-quantified version of Q3, which currently rests on a 15-vs-15 descriptive split plus one-at-a-time partials. `gamma_int > 0` would mean letter sounds convert to word reading _more_ strongly for children with better phonological memory — exactly the Q3 hypothesis. No new factory code: `erbto` is a covariate rather than a `MEASURES` symbol, so it takes `moderator_is_covariate=True`, the path `mech-073`/`173` already use for age |
| `lrp-rli-mech-105` | `mechanism` | `LS → WR` **moderated by `NW`** (or by an off-floor `NW` indicator) | Tests whether the letter-sound link is carried by the children who can decode; `NW` is a `MEASURES` symbol so this is the plain `mech-071` pattern. But `NW` is a **mediator**, so the moderation is descriptive only, and the floor limits power — the off-floor binary version is likely better-powered                                                                                                                                                                                                            |

### Tier 3 — needs new factory or pipeline work

1. ~~**A joint `{WR, NW}` levels model.**~~ **Built** (#421 Tier 3 (1)), as two models rather than one, because the two quantities live on different parameterisations. [`lrp-rli-jm-001`](../docs/models/lrp-rli-jm-001/) is the per-wave levels model, matched to `ca-010` / `ca-011`, and reports Q2's share-retained as an identified within-model deterministic. [`lrp-rli-jm-002`](../docs/models/lrp-rli-jm-002/) is the phase-stacked ANCOVA companion, matched to `mech-096` / `mech-101`, and re-reports the Tier-1 Δ on the parameterisation the [decoding-specificity note](202607172358-findings-decoding-specificity.md) defines it on — reading that Δ off a levels fit would have swapped the estimand while claiming to identify it. Both carry an LKJ cross-outcome dependence block: a single shared child intercept with a fixed loading of 1 on both logits would leave the two likelihood legs factorising, and so would identify nothing. Numbers land in both notes once the fits pass the gate at `--config reporting`.
2. **A floor-tolerant second mediator leg** in `build_two_mediator_model`, which would finally unlock the chained `LS → NW → WR` decomposition (`med-081`, specced then withdrawn at build time). The factory currently requires an autoregressive baseline per mediator leg and hard-codes the first leg to `L`; `NW` is post-only and ~72% floored at t1, so it cannot supply one. A Bernoulli off-floor second leg — reusing the existing `bernoulli_offfloor` likelihood — is the obvious route.
...
### Not a model, but the cheapest next step

**Same-wave rather than t1-baseline `RW`/`SP` in the Q3 partials.** The probe enters those two as t1 baselines competing against same-wave skills, which is a timing asymmetry that plausibly explains why they collapse in the joint fit. This is a one-line change to the probe, not a new model, and it should be done before `mech-102`/`103` are specced.
