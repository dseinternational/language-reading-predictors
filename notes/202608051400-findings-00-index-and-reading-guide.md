...
**All 194 models in this run pass the convergence gate, with zero divergent transitions across the whole suite.** This is the first run in which every model clears computation. A separate robustness gate adopted on 2026-08-05 — after these fits were made — then withholds two of them from release: the ITT floor-rule outcomes phonetic spelling and nonword reading, whose required treatment-prior grid is not yet in place (note 01). Three things also changed materially:
