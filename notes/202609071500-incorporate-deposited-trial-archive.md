...
1. **Seven Quarto render tests cannot pass on Windows: they build a POSIX-shaped minimal subprocess environment and omit every Windows equivalent.** All five in `test_mechanism_report_forms.py` and both end-to-end render tests in `test_report_restructure.py`. Each forwards `PATH`, `LANG`, `LC_ALL`, `TMPDIR` and `SYSTEMROOT`, and sets `HOME`. Three variables are then missing, and each kills the render at a different point before any assertion is reached:
   ...
   Forwarding `TMP`, `TEMP`, `USERPROFILE`, `HOMEDRIVE`, `HOMEPATH`, `APPDATA`, `LOCALAPPDATA`, `COMSPEC`, `PATHEXT` and `WINDIR` takes the same fixture from exit 1 to `Output created: index.html`. Confirmed pre-existing: the failures reproduce with every change here stashed, and the observed count is order-dependent (four of the five mechanism tests passed in one full-suite run, all five fail in isolation), which is what a retried temp-directory probe falling back to the working directory produces. Quarto itself is healthy — a minimal document with a Python chunk renders, and so does the whole `docs/report/` book. Worth its own issue: these tests guard the shared report partials the technical report will be built from, and on Windows they have been passing vacuously or failing for reasons unrelated to what they assert.
