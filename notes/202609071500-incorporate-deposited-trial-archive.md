...
- Docs updated: `data/readme.md` (rewritten, with a files table and the three unique contributions), `METHODS.md`, `docs/models/README.md`, the fit script's help text and the importer's docstring. The three synced agent instruction files needed no change — their "archive" references are to archived `config.json` files and to the 54-child analysis file, both still accurate.
...
1. **Three Quarto render tests fail on a clean tree** — `test_mechanism_report_forms.py::test_linear_report_claims_no_curve_prior_or_steepest_interval` and both end-to-end render tests in `test_report_restructure.py`. Confirmed pre-existing by stashing every change and re-running. Quarto itself is healthy: a minimal document with a Python chunk renders, and so does the whole `docs/report/` book. Worth its own issue, since those tests guard the shared report partials.
