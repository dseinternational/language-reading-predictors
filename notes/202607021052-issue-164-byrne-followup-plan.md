...
The 96-versus-97 extract lineage is reconciled, `basmat` is confirmed from the retained source's native `BASMAT3`-`BASMAT5` fields, and the absence of immediate and delayed visual recall from every retained source field is accepted as an explicit reproduction limit (`notes/202608161340-byrne-source-provenance-reconciliation.md`; `notes/202608161554-byrne-basmat-and-visual-recall-audit.md`). Confirm the remaining `basspel`, `woco` and `basnum` denominators before publishing models that use them. The broad denominator/Negative-Binomial sensitivity passes for the Phase-A growth read-outs but does not identify a ceiling or justify automatic promotion of the unbounded likelihood (`notes/202608161900-byrne-denominator-likelihood-sensitivity.md`). The denominator-free paired-growth Bayesian bootstrap passes for `woco` and `basnum` but preserves a strict `no_go` for `basspel` because one near-zero contextual contrast changes median sign; all five 89% intervals overlap, so this is an audit-rule failure rather than evidence for a stable reversed direction (`notes/202608161945-byrne-participant-bayesian-bootstrap.md`). For the reciprocal reading-language-auditory-memory question, the graph is adopted but both candidate models failed the pre-fit recovery gate (`notes/202608141812-byrne-lcsm-feasibility.md`), so no Phase C model is registered. Bridge and pooled models remain a separate methodological track.
