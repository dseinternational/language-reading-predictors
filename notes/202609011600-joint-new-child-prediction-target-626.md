...
**`joint` (RLI), `lrp-rli-itt-012`, no residual block.** With no child latent the integrated term _is_ the conditional one, and the computation reproduces the stored fit's Pareto-$k$ maximum of 0.701 exactly at the full 36 000 draws. That agreement is the engine's correctness check as much as it is a result. It also shows what thinning costs: the same fit's maximum $k$ read 0.98 at 1 500 draws and 0.65 at 4 000, while the ELPD moved by less than 0.4 across all four settings. The shape estimate is a tail quantity; the ELPD is not.
