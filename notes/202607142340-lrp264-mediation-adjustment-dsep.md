...
## Addendum 2026-07-17 — the six siblings' exogenous confounders (full-suite review)

The full-suite adjustment-set review (`notes/202607172000-adjustment-set-review-full-suite.md`) revisited this family with a different question than the `E`/`R` one settled above: **do the six exploratory siblings that lack the exogenous mediator-outcome confounders `HS`/`SP`/`RW` (MED-066/068/074/075/076/080) match the flagships' observable-confounder handling?** They did not — the #259 revision added `HS`/`SP`/`RW` to MED-059/062/064 (and this note confirmed MED-086/087 need none), but the six siblings were carried forward on their pre-#259 sets, and three docstrings (MED-066/075/076) claimed set-identity with a flagship that was no longer true.

Adding these covariates cannot achieve identification — the `IS` witness above is untouched, so every model stays ID-2 — so this is a bias-reduction / cross-model-comparability choice, not an identification one. For parity the six were extended on 2026-07-17 to carry the DAG-parent exogenous confounders of their mediator(s): MED-066/075/068 `+ HS, SP, RW`; MED-080 `+ HS, RW`; MED-074 `+ SP, RW`; MED-076 `+ HS, SP` (which makes it genuinely identical to MED-059). The baseline skills (`E`/`R`/`L`) were retained per the derivation above, the docstrings were corrected, and the six were re-fit at `reporting`. The per-model "Fitted set (mapped)" column above therefore records the **pre-2026-07-17** sets for those six rows; the live `SPEC.adjustment` lists (which the CI guard reads) are authoritative. The CI assertions are unaffected: the sets stay admissible (no `IG`-descendant is added — `HS`, `SP_1`, `RW_1` are pre-treatment) and the mediator-outcome witness stays unblockable. Following the #246 comparison pattern above, the expected effect is that the direct path shrinks and the mediated proportion rises for the six, with the indirect (letter-sound / code-route) share little changed.
