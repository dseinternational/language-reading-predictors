...
**Cannot say.** That any of this is causal, or even, for two of the three targets, that it is an adjusted association in the DAG's terms. Under the lagged graph the `WR → LS` coupling is identifiable with a small set, and that set is used here; `WR → PA` needs 8–11 adjusters; and **`WR → NW` is not an edge in the graph at all**, which is the open decision in #428 — so the nonword result, the strongest one here, currently has no identified estimand behind it. Direction is not established either: a child beginning to decode will also read more words, and this design cannot separate the two. Latent general ability sits inside every number.

## What would be worth building

**One model, and it is blocked on a decision.** The nonword result is the only one here strong enough to be worth registering, and its shape (concave, off-floor) is not what the current `mechanism` family fits by default. It is specified in **#433**, which turns on #428: if the `WR_t → NW_{t+1}` edge is rejected, the model ships as an explicitly descriptive fit or not at all.

**Nothing for letter sounds or blending.** Both answers are negative and the negative is uninteresting to model further: the existing `lcsm-082` already estimates the reading → blending coupling, and #429 records the optional reverse-coupling LCSM for letter sounds.
...
- Issues: #428 (the `WR → NW` DAG decision this note's main result depends on), #433 (registering the nonword model), #429 (the optional reverse-coupling LCSM).
