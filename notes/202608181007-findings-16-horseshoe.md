...
Two of these fits reach that clean result only because their modules declare a raised acceptance target in-module — `hs-001` and `rlm-hs-003` set `target_accept` 0.999, and the two remaining historical fits set 0.99. Any refit must read `config.json` → `sampling.target_accept` rather than assuming the reporting preset's 0.95, since passing a uniform lower value would silently loosen these.
